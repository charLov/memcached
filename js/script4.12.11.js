...
var test = async function () {
	var txt = 'sougouwiki.com/search?keywords=%C1%B4%B9%BB%C0%B8%C5%CC5%BF%CD%A4%B7%A4%AB%A4%A4%A4%CA%A4%A4%C5%C4%BC%CB本作の主演は朝日りんちゃん！！快活な笑顔と短髪の組み合わせが最高にマッチしているGカップ巨乳の高身長美少女だ！！竹を割ったような明るい性格でスタッフとも楽しくやり取りが出来るポジティブガール！！\n165cmという恵まれた天からのギフトに加え、美しい90cmバストとスタイルはジム通いで鍛えた努力の証でもある。可愛いりんちゃんとその美しい裸だけを見たいという要望\nも多く、ようやくその声に応えられると笑顔で語ってくれた。念願叶い撮影したイメージ作品は楽しかったと笑顔のりんちゃん、スムーズに進行し沢\n山の魅力的な姿が収穫できた！！ファン待望のファーストイメージ、スタイル抜群快活美少女の笑顔は朝日のように明るく照らし生きるエナジーを与える！！';
	//console.log(decodeURIComponent(txt))
	var aid = 'char-990';
	var apiid = 'QGmmRD0VGaC7yVTwSc2H';
	var keyword = '「こんなのはじめてです…」初めてだらけの性感開発3本番スペシャル！！堀沢茉由';
	var req = 'https://api.dmm.com/affiliate/v3/ItemList?api_id=' + apiid + '&affiliate_id=' + aid + '&site=FANZA&service=digital&floor=videoa&hits=10&sort=date&keyword=' + keyword + '&output=json';

	//var txt = await promisePHP(encodeURI(req),'readUrl_1');
	//myConform(txt);
	var res = await myConfirm(txt);
	alert(res);
	await myAlert('rerera');
	const result = await myPrompt("The answer is");
	alert(result);

	console.log('myConfom ends')
...
//https://duga.jp/ppv/orustaksoft-0095/
  ...
  if (urlData.match('www.dmm.')) {  // DMM movie
    ...
    var url = encodeURIComponent(urlData);
		url = '<a href="https://al.dmm.co.jp/?lurl=' + url + '%2F&af_id=char-001&ch=link_tool&ch_id=text" rel="nofollow" target="_blank">Dmmで詳細を見る<br><br></a>';
    $("#img").val(url);
    var cid = urlData.match(/\/cid=(.+?)\//);
    console.log(urlData);
    if (cid) {
      console.log('DMM cid = ' + cid[1]);
			if (urlData.match(/www.dmm.com/)) {
				console.log('Go mainDmm2',cid[1])
				mainDmm2(urlData);
			} else if (urlData.match(/www.dmm.co.jp/)) {
				console.log('be going mainDmm')
				mainDmm(cid[1]);
			}
    }
...
*/
	...
	if (urlData.indexOf('nyoshin.com' >= 0)) {
		...
		if (pId) {
			...
			var img = $('#img').val();
			if (img.indexOf(imgUrl) < 0) $("#img").val(url + img);
  ...
  if (urlData.indexOf("www.mgstage.com") >= 0) {
    var sp = urlData.split('/');
		...
		if (txt.indexOf('mgstage.com') < 0) {
			txt += '<a href="' + urlData + '?aff=8IMROFDGQICYG35O5PYD6L72F3" target="_blank">MGSで詳細を見る</a><br><br>';
			$("#img").val(txt);
		}
		txt = $('#append').val();
		if (txt.indexOf('mgstage.com') < 0) {
			txt = $('#append').val() + '<script type="text/javascript" charset="utf-8" src="https://www.mgstage.com/js/mgs_sample_movie.js?p=' + sp[5] + '&r=1&c=8IMROFDGQICYG35O5PYD6L72F3"></script>' + txt;
			$('#append').val(txt);
		}
    return;
  }
  ...
  for (var no = 0;no < max + 1;no++) {//no = object No
    for (var i = 0;i < max;i++) {//i = search No
      ...
      if (num == no) {
        ...
        sub += '<a href="' + src[1] + '" data-lightbox="example-1"><img src="' + src[1] + '" alt="' + alt + '" style="max-width: ' + size + 'px; max-height: ' + size
...
//    else {no1 = no[2]; no2 = no[3]}
  ...
  img = '<a href="https://al.dmm.co.jp/?lurl=https%3A%2F%2Fwww.dmm.co.jp%2Fmono%2Fdvd%2F-%2Fdetail%2F%3D%2Fcid%3D'
    + pNo + '%2F&af_id=char-001&ch=link_tool&ch_id=text" rel="nofollow" target="_blank">Dmmで詳細を見る</a><br><br>';
  p = $('#img').val();
  if (p.indexOf(img) < 0) $('#img').val(img + p);
}
...
var mainDmm = (url,retry = 0) => { // （一覧表示後に）FANZAの商品IDで検索。
  ...
  if (!cids) { aModal('商品番号が見つかりません', 'mainDmm'); return; }
  var cid = cids[1];
  asDmm(cid);
...
//<td width="100%">2021/02/25</td>
	...
	if (cid) {
		...
		imgUrl = '<a href="https://al.dmm.com/?lurl=https%3A%2F%2Fwww.dmm.com%2Fmono%2Fdvd%2F-%2Fdetail%2F%3D%2Fcid%3D'
			+ cid[1]
...
var gakunen = (sd, bd) => {
	...
	// nenrei = 6 bd < 0402 => 1  bd > 0401 0
	//  0201 umare 6 sai 0401 6 sai genzai 0301 0 genzai 0601 1
	//  0315 umare 6 sai 0401 7 sai genzai 0301 1 genzai 0601 1
	//  0601 umare 6 sai 0401 6 sai genzai 0301 0 genzai 0501
	//  0201 umare 6 sai genzai 0601 1
	//  0315 umare 6 sai genzai 0601 1　その年の4月1日の年齢-6　今日が1月から4月まで−１学年
	//  0801 umare 6 sai genzai 0601 1
	//   today  0301 0101 < bd < 0301 gakunen 0  4gatukara 1 nensei
	//               0301 < bd < 0401 gakunen 2  4gatukara 2
	//               0402 < bd < 1231 gakunen 1  rainen 2 nensai
	//   today  0601 0101 < bd < 0401 gakunen 1
	//               0402 < bd < 0601 gakunen 0
	//               0602 < bd < 1231 gakunen 1   rainenn
	//bd > 0401 gakunen 1
	//               bd < 0301 gakunen 1
	//
	// nenrei = 7 bd < 0402 => 2  bd > 0401 1
	//   today  0301 
}
...
var titleChange = (tNo) => {
  ...
  var title = $('#' + tNo).val()
		.replace(/ （ブルーレイディスク）| （DOD）|【数量限定】|【ベストヒッツ】|【アウトレット】/g, '');
  for (var t of titles) {
...
//  var filename = 'https://www.gekiyasu-dvdshop.jp/products/list.php?mode=search&name=' + pNo;
  ...
  if (!$('#img').val().match('<a href="http://www.gekiyasu-dvdshop.jp'))
    $('#img').val('<a href="http://www.gekiyasu-dvdshop.jp/affiliate/link.php?'
      ...
      + filename + '" target="_blank">激安DVDショップで詳細を見る<br><br></a>' + $('#img').val());
...
//        console.log(txt.length);
		...
		if (mode == 'none') {
			...
			//let c = confirm('読み込みますか？！！！！(List)\n\n\n' + rText)
			await myAlert(rText);
			let c = await myConfirm('読み込みますか？！！！！(List)\n\n\n' + rText);
			if (!c) return;
			//var c = await myAlert('読み込みますか？！！！！(List)\n\n\n');
			//if (!c) return;
			for (var line of txt) {
...
var readOkashiLemon = (res,osnap,pid) => {
  ...
  for (var i in arr) {//arr.length;i++){
    ...
    var ar = arr[i].split(',');
    if (osnap) {//okashi
			txt = imgShaping(ar[15], size).replace(/<\/a/g,'お菓子系で詳細を見る<br><br></a') + '\n' + imgShaping(ar[16], size);
    } else {//lemonUp
...
var okashiK = async (urlData,osnap = false) => {
	...
	var res;
	if (osnap) res = await promisePHP('OkashiK.csv', 'read');
	else res = await promisePHP('LemonUP.csv', 'read');

	if (res) readOkashiLemon(res, osnap, pid[1]);
...
easeSurugaya = async (urlData) => {
  ...
  if (img.indexOf(url) < 0) {
    url += '<img src="https://www.suruga-ya.jp/database/pics/game/' + spd[1] + '.jpg" alt="' + $("#title").val() + '" style="border: none;" />駿河屋で詳細を見る</a><br><br>';
      $("#img").val(img + url);
  }
...
$('.orgButton').on('click', function (event) {
  ...
  if (id) {
    ...
    if (url.length > 5) {
			$('#' + 'orgMinna').append('<div class="orgChild" style="background-color: white;"><iframe src = "' + url + '" style = "width: 100%; height: 1000px;"  allowfullscreen = "true" ></iframe ></div>');
    }
  }
})
...
$('.org-label').on('click', function (event) { 
	...
	if ( (url.match(/http.*?:\/\//))) {
			$('#' + 'orgGirl').append('<div class="orgChild" style="background-color: white;"><iframe src = "' + url + '" style = "width: 100%; height: 1000px;"  allowfullscreen = "true" ></iframe ></div>');
	}
  ...
  if (tab) {
    ...
    if (tab[2] == 'Url') {
        ...
        case 'Girl':
          ...
          break;
				...
				case 'Duga':
					...
					break;
				case 'Fc2':
					url = 'https://adult.contents.fc2.com/search/?q=' + show;
					break;
				case 'Ichigo':
				...
				case 'Google'://ｇSearch
					...
					wObj = window.open(url, '_blank','menubar=1,toolbar=1,location=1,status=1,width=800,height=800');
					return;
        default:
          url = ''; show = '';
          break;
      ...
      if (show == '') return;
		...
		wObj = window.open(url, '_blank', 'width=800,height=800,');
...
var myConformF = async (mes) => {
	console.log(mes)
	const result = await myConfirmF(mes);
	alert(result); // ①
}
