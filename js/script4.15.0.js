...
ファイルがないときには「tmp_」ファイルを読み込むようにすること
...
var test = async function () {
	...
	mass.mes = 'message';
	var res =  await myPrompt('kore?', 'shokichi', 'tokucha', 'wawawa', mass);
	console.log('myPrompt("kore?",res confirm message after await myCOnfirm',res, mass)
	var res = await myAlert('kore????', 'title!!!', 'Yes!!!');
	console.log("await myAlert('simple alert !!')")
	var ok = await test1('promise mess');
	console.log('OK= ',ok);
	console.log('res confirm message after await myCOnfirm', res, mass);
	var res = await myConfirm('message????', 'continue2????', 'yes????', 'no????', 'yobibibib');
	console.log('await myConfirm("message????")','before sabveTEmpfile');
	//await saveOnClick();
	console.log('after savbeTEmpfile');
};
var test1 = (mess) => {
	return new Promise(async (resolve) => {
		await myConfirm('Symple confirm',mess);
...
//		$('.trumbowyg-fontfamily-button').addClass('dark');
	...
	setTimeout(() => {
		...
		console.log('setTime 5000')
		////csrChange('auto','main')
	}, 5000);

	setTimeout(function () {
		console.log('setTime 8000')
  ...
  str = str.replace(/[ ／]/g,'、');
    ...
    + 'search_result.php?search_scope=actress&search_word=';
		...
		if (ninzu) {// actless list
			...
			if (!actless) {await myAlert('error No actless'); continue;}
        ...
        setTimeout(function() {
          ...
          if (m) {//alert(m)
            minnaPHP(m[1],long);
            console.log('m[1] = ',m[1]);
      //      nDim.push(m[1]); //scrapeMinna(m[1],long);async 
          }
          else {// alert(names[i])
...
var mainDmm = async (url,retry = 0) => { // （一覧表示後に）FANZAの商品IDで検索。
  ...
  setTimeout(function() {
    fetch(urlx,{method: "GET",}).then(response => response.text()).then(text => {
      ...
      if ((cmt) && ($('#cmt').val().length < 1)) $('#cmt').val(cmt[1].trim()); else alert('no comment !');//発売日：</td>
...
var setTrumbowyg = (area, color = '#cc6', target = null) => {
	...
	$('#' + area).trumbowyg({
		...
		btnsDef: {
			...
			personal: {
				dropdown: ['clear','myNote','myAdjust','count'],
				title: '独自関数',
				ico: 'person'
			},
  ...
  //　変更されたとき
	.on('tbwchange', function () {
		...
		var count = $('#' + area).trumbowyg('html').length.toLocaleString();
		console.log('trumbowyg cout ', count);
		$('#trumbowyg-count-' + area).html(count + '字');
	})
	//////// fullscreen //////////		
...
var saveOnClick = (mode = 'append', numStr = '20') => {
  ...
  if (fileName == '.txt') fileName = 'untitled.txt';
	...
	if ((numStr == 'temp') || isNaN(num)) {
		fileName = 'tmp_data/tmp_' + fileName;
		num = 999;
		mode = 'overwrite';
		if (numStr != 'temp') await myAlert('saveTempFile not temp num=' + numStr + '\n\nfilename =' + fileName);
	}
  //var date = new Date();
	var dataFileName = 'data/' + fileName.trim();
  console.log(dataFileName);
  var ver = ('000' + num).slice(-3);
	//  saveTempFile('data/' + dataFileName.trim(),allToText(tree,ver),mode,num);
	//}

	//var saveTempFile = async (dataFileName, txt, mode = 'write', num = 1) => {
	if ((mode == 'append') || (mode = 'clear')) {
		...
		if (myByte) {
			...
			if (filesize > maxByte * 1024 * 1024) {
				if (await myConfirm('ファイルサイズが' + maxByte +'MByteを越えました。\n\nfilesiza=' + filesize + 'bytes', 'バックアップ保存しますか？', 'バックアップ保存', '上書き保存')) {
					var his = 0;
					var searchFileName = 'complete/' + fileName.slice(0, -4) + '(' + his + ').txt';
					var isFileName = await promisePHP(searchFileName, 'isfile');
					while (isFileName.indexOf('が存在します') >= 0) {
						his++;
						searchFileName = 'data/complete/' + fileName + '(' + his + ').txt';
						isFileName = await promisePHP(searchFileName, 'isfile');
						if (his > maxBackup) break;
					}
					aModal('現在のバックアップ数', his.toString());
					await promisePHP('data/' + fileName, 'move', searchFileName);
				}
				mode = 'overwrite';
			}
		}
	...
	var res;
	if (mode == 'clear') res = await promisePHP(dataFileName, 'append', allToText(tree,ver), num);
	else res = await promisePHP(dataFileName, mode, allToText(tree,ver), num);
	if (res) {
		var date = new Date();
		...
		if (num == 999) aModal('仮保存しました',time);
			...
			await promisePHP('data/tmp_data/tmp_' + fileName, 'delete');
			aModal('追加保存しました', time);
		}
		else if (mode == 'move') aModal('移動', res.match(/(移動.+?)\n/)[1]);
		else if (mode == 'delete') aModal('削除', res.match(/(削除.+?)\n/)[1]);
		else if (mode == 'clear') {
			res = await promisePHP('data/' + fileName, 'move', 'data/complete/' + fileName);
			await promisePHP('data/tmp_data/tmp_' + fileName, 'delete');
...
var autoSave = (minute = 10,char = 1000,stop = false) => {
  var count = 1;
  ...
  var autoS = async () => {//　文字数
    if ($('#changed').html().indexOf('*') < 0) return;
    $('#changed').html('');
    var nChar = allToText().length;
    if ((nChar - old > char) || (old - nChar > char)) {
      console.log('autosave count = ' + count,nChar,old,char);
      await saveOnClick('append',String(count++));
      old = nChar;
    } else {
      await saveOnClick('overwrite', 'temp');
    }
  }

  $('#autoSaveNumber').val(setInterval(autoS, minute * 1000));
...
var readSavedFile = async () => {// only "on load" now 
	var fileName = $('#title').val().trim().substr(0, 32).replace(/[:\s|\/]/g, '_') + '.txt';
  if (fileName == '.txt') return;
	var savedFileData = await promisePHP('data/' + fileName,'isfile');
	var tempFileData = await promisePHP('data/tmp_data/tmp_' + fileName,'isfile');

	if (tempFileData.indexOf('は存在しません') > 0) {
		if (savedFileData.indexOf('は存在しません') > 0) return;// no file
			fetchTempFile('data/' + fileName, 'modal');// only saved file
	} else {
		console.log('tempFileData',tempFileData)
		if (savedFileData.indexOf('は存在しません') > 0)// only temp file
			fetchTempFile('data/tmp_data/tmp_' + fileName, 'modal');
		else { //both exist
			var tempFileTime = tempFileData.match(/(\d{4}\-\d{2}\-\d{2}\s?\d{2}:\d{2}:\d{2})/)[1].replace(/-/g, '/');
			console.log('tempFileTime', tempFileTime)
			var savedFileTime = savedFileData.match(/(\d{4}\-\d{2}\-\d{2}\s?\d{2}:\d{2}:\d{2})/)[1].replace(/-/g, '/');
  ...
  $('#savedFileName').val(fileName);
	if (!await fetchTempFile('data/' + fileName, 'modal')) {
		await fetchTempFile('data/tmp_data/tmp_' + fileName, 'modal');
	}*/
}
...
var modalSaved = (rText,page = 0) => {
  if (rText.match('failed to open stream')) {
	...
	$('#txtSaved').val(rText);
	//console.log('modalSved page=>',page,'rText => ',rText)
...
var showSavedData = async (num) => {
  ...
  var same = true;
	...
	if (startDate) {
		...
		var temp = await myConfirm('データがありません num => ' + num + '\ntxtBlock => ' + txtBlock[num], 'filename = ' + filename, 'Tempファイルから読み込み', '閉じる');
		if (temp) {
			await fetchTempFile(filename.replace('data/','data/tmp_data/tmp_'), 'modal');
		}
		return false;
	}
  ...
  if (same) {
    aModal('最終保存と同じ','modalSaved');
		$('#same').text('現在のページと同じ');
		$('#same').css('color', '#4a4');
		$('#loadFileButton').attr('disabled', true);
    return 'same';
  }
  $('#same').text('現在のページと異なります');
	$('#same').css('color', '#a44');
	$('#loadFileButton').removeAttr('disabled');
	return 'different';
}
...
async function scrapeCarib(urlData) {
  ...
  if (title && ($('#title').val().length == 0)) {
    $('#title').val(title[1].replace(' - 無修正動画 カリビアンコム ','').normalize( "NFC" ));
  }
	...
	for (var i = 0; i < prof.length; i++) {
		...
		if ((item) && (item.length == 4) && (prof[i].indexOf('生年月日') < 0)) {
			switch (item[2].trim()) {
				...
				case 'タグ':
				case 'ジャンル':
					var tags = ['オリジナル動画','AV女優','ベスト・オムニバス','淫語', '微乳', '美脚','モデル系','熟女/人妻', 'パイパン','痴女', '口内発射','水着', 'ロリ', '美尻', 'スレンダー', '巨乳','放尿', '美乳','ハード系','レズ','パイズリ','ごっくん','アナル', '手コキ', 'イラマチオ', 'マングリ返し','コスプレ','ぶっかけ','69','乱交','顔射','オナニー','潮吹き','初裏', '中出し', '生ハメ・生姦', 'バイブ', 'クンニ','ブルマー','制服', 'フェラ','チオ','3P', 'ストリーミング対応', '1080p', 'xxfps','フェチ','単体作品','カップル','主観'];
					for (var tag of tags) {
						//console.log('tag', tag);
...
$('#loadFromFile').on('click', async () => {
	...
	if (slash) {
		...
		//alert(filename); console.log(filename)
		var rText = await promisePHP('data/' + slash[1], 'read');
		if (modalSaved(rText))
			await myAlert('data/' + slash[1] + '\n\nmodal表示エラー','ファイルが開けません');
	} else {
