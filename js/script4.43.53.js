...
$('.clear').click(function () {
	...
	if ($('#changed').html().indexOf('*') < 0) {
		saveTempFile('data/' + fileName, 'data/complete/' + fileName, 'move');
    clearAll();
  } else {
		$('#selectProcess').val('クリア');
    $('#dir').text('data/');

    saveTempFile('data/complete/' + fileName,allToText(),'write','clear');
...
var setTrumbowyg = (area, color = '#cc6', target = null) => {
	...
	$('#' + area).trumbowyg({
		...
		btnsDef: {
			...
			blockquote: {text: '<blockquote' + para + '>引用</blockquote', key: '0'},
...
var showSavedData = async (num) => {
	...
	var nextDate;
	if (nextBlock) nextDate = nextBlock.match(/<!-- data start (\d{4}_\d{2}_\d{2} \d{1,2}:\d{1,2}:\d{1,2})/);
