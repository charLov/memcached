...
setTimeout(function() {
    ...
    deleteAllCookies();
    $('#changed').html('');
    console.log('on start','idChanged');
  },1000);
...
/*  var nenrei = $.cookie('nenrei');
  ...
  if ($.cookie('dmmImage')) {
    displayImage($.cookie('dmmImage'),'onload');
  }
    ...
    if (urlData.indexOf('www.dmm.com') > 0) {
      ...
      if (sp) {
        var url = '<iframe type="text/html" src="https://www.dmm.com/service/digitalapi/-/html5_player/=/cid='
          + sp[1]
          + 'mtype=CARdWwNE/service=mono/floor=dvd/mode=/" id="DMMSample_player_now" scrolling="no" border="0"'
          + ' style="border:none;max-width: 100%;" allowfullscreen="" width="720" height="480" frameborder="0"></iframe>'
          + '<!--video width="100%" controls poster="'
          + imgUrl + '"><source src="https://www.nyoshin.com/d2p/movie/'
          + sp[1] + '/sample.mp4"><p>ご使用のブラウザでは動画再生に対応していません</p></video>';

        displayImage(imgUrl,'append');
      ...
      if (urlData.indexOf(org[i], 0) > 0) {
        ...
        if (urlData.match('nyoshin')) {
          ...
          if (pId) {
            ...
            a += '<video width="100%" controls poster="'
              + imgUrl + '"><source src="https://www.nyoshin.com/d2p/movie/'
...
var allToText = () => {
  ...
  txt += '<!-- ofile -->' + $('#ofile').val() + '\t';
  txt += '<!-- n -->' + $('#n').val() + '\t';
  txt += '<!-- wEditor -->' + $('#wEditor').trumbowyg('html') + '\t';
...
var addTag = (txt) => {
  ...
  var line = '';
  for (var i = 0;i < txts.length;i++) {
    if (txts[i].match(/^<h.>/)) {line += txts[i] + '\n'; continue;}
    var quote = txts[i].split(/__|＿＿/g);
    if (quote[0]) {
      line += '<span class="quote">' + quote[0] + '</span>';
    }
    if (quote[1]) {
      line += '＿＿<span class="note">' + quote[1] + '</span>';
    }
    line += '\n';
  }
  return line;
...
//      var act = json.match(/id="performer">\s.+>(.+)<\/a/);
      ...
      if (imgUrl) {
       displayImage(imgUrl[1].replace('ps.jpg','pl.jpg'),'p-ID');
     }
...
var displayImage = (imgUrl,e = 'unknown') => {
  var el = document.getElementById('orgImage');
...
var hotPHP = (filename) => {
  ...
  if (pid) {
    ...
    var append = $('#append').val();
    if (append.length > 0) append +='\n\n\n';
    append += '<video width="100%" controls poster=""><source src="http://my.cdn.tokyo-hot.com/media/samples/'
  ...
  xmlhttp.onreadystatechange = function() {
    ...
    if (xmlhttp.readyState == 4) {
      if (xmlhttp.status == 200) {
        ...
        if (mNo && img) {
          if (img) {
            displayImage('https://www.gekiyasu-dvdshop.jp' + img[1],'Scrape Geki');
          }
          var isV = res.match(/<video[\s\S]+?(<source src=[\s\S]+?<\/video>)/)
          if(isV) {
            var movie = $('#append').val(); if (movie != '') movie += '\n\n';
            movie += '<video width="100%" controls poster="';
...
//  mainUrl = "https://www.dmm.co.jp/age_check/=/declared=yes/?rurl=" + mainUrl;
//https://www.dmm.co.jp/mono/dvd/-/detail/=/cid=hunta954/?i3_ref=search&i3_ord=1
//https://www.dmm.co.jp/mono/-/search/=/searchstr=huntb112/ script.js:4312:23
//https://www.dmm.co.jp/rental/ppr/-/detail/=/cid=2huntb112/?i3_ref=search&i3_ord=1
//https://www.dmm.co.jp/digital/videoa/-/detail/=/cid=huntb00112/?dmmref=digiprice2
//https://www.dmm.co.jp/mono/dvd/-/detail/=/cid=huntb112/?i3_ref=search&amp;i3_ord=2">                 <span class="img"><img src="//pics.dmm.co.jp/mono/movie/adult/huntb112/huntb112pt.jpg" alt="『偽装デキ婚』数年付き合っている彼氏と玉の輿婚をしたいけどフラれるかもしれないと恐れるアラサーの義姉がボクに種付け依頼で連日中出しを求めて…"></span>                 <span class="txt">『偽装デキ婚』数年付き合...</span>             </a>         </p>          <p class="sublink">             <span><a href="https://www.dmm.co.jp/mono/-/search/=/searchstr=huntb112/">通販</a script.js:4317:23
//mainUrl = 'https://www.dmm.co.jp/mono/dvd/-/detail/=/cid=huntb112/?_ga=2.44631012.1589318822.1634907708-486149399.1634907708'
...
//          saveToFile(xmlhttp.responseText,name + filename);
          ...
          return;
//jsonToProfile(xmlhttp.responseText,true);
        }
        xmlhttp.onerror = function() {
          csrChange('auto','minnaPHP-2');
          aModal("error Scrape status = " + xmlhttp.status);
        }
      }
    }
    return;
...
var scrapeToProf = (txt,flag) => {
  ...
  if (birth) res += strBirth($('#date').val(),birth[1]) + '\n'; if (flag) res += '\n';
...
var readTempFile = (fileName,i = 'none') => {
  csrChange('wait','readTempFile');
  var xmlhttp = new XMLHttpRequest();
...
var saveTempFile = (fileName,txt,count = 1) => {
  var xmlhttp = new XMLHttpRequest();
...
//                  onclick: 'readFeel("' + file + ',' + no + '")',
          ...
          onclick: 'readTempFile("' + dir + filename + '","' + i + '")',
        }))
        .append($('<button>',{
          class: 'col-sm-1 btn btn-sm float-right btn-outline-danger',
//          text: '<i class="fas fa-pen-nib"></i>',
...
var moveFeelP = (fileName,flag = true) => {
  ...
  if (flag) txt = prompt(fileName + ' を移動します',txt);
  if (!txt) return;
  var url = "saveTemp.php"//?mode=write?filename=" + fileName;
  xmlhttp.onreadystatechange = function() {
    if (xmlhttp.readyState == 4) {
...
var autoSave = (minute = 5,char = 500,stop = false) => {
  var count = 1;
  ...
  setInterval(countUp, 60000 * parseInt(minute));
...
scrapeHeyzo = (res) => {
  ...
  if (video) {
    ...
    var sample = '<video src="https:' + video[1] + '" width="100%" controls ';
...
scrapeSurugaya = (res) => {
  ...
  if ($('#date').val() == '') $('#date').val(item[3].replace('-','/'));
  ...
  img.src = 'https://www.suruga-ya.jp/pics_light/boxart_a/'
    + pid + '_1.jpg';
  img.id = 'dmmImage';
  el.appendChild(img);
}
...
var readOkashiLemon = (res,osnap,pid) => {
  console.log('lenth',res.lenth,'osnap',osnap,'pid',pid);
  var append = $('#append').val();
  var arr = res.split(/\r\n|\r|\n/);
  var txt;
