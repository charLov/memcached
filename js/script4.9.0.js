...
//    area.focus();
  ...
  if (!area) {
    aModal('エレメントが選択されていません','ダブルクリック');
    return;
  }
...
var mainDmm = (cid,retry = 0) => { // （一覧表示後に）DMMの商品IDで検索。
...
//  var path  = imgUrl;
  ...
  if (!imgName) {
    ...
    var exist = $(e).html();
    console.log('get content of',e,exist,imgUrl,imgName)
    ...
    if (xmlhttp.readyState == 4) {
      if (xmlhttp.status == 200) {
        ...
        switch (save) {
          ...
          case 'surugaya': scrapeSurugaya(res); 
