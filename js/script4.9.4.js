...
var mainDmm = (cid,retry = 0) => { // （一覧表示後に）FANZAの商品IDで検索。
  ...
  xmlhttp.onreadystatechange = function() {
    if (xmlhttp.readyState == 4) {
      ...
      if (xmlhttp.status == 200) { // LIST get
        ...
        var cmt = txt.match(/<p class="mg-b20">([\s\S]+?)<\/p>/);
...
var scrapeDmm = (pNo,retry = 0) => { // 商品型番から一覧表示　その後
  if (retry > 5) {
...
//  saveToFile(res,'scrapeSurugaya.htm');
  ...
  if (img1) { for (var img of img1) displayImage(img); }
  var img2 = res.match(/https:\/\/www.suruga-ya.jp\/database\/pics_light\/game\/.+?jpg/g);//front
  if (img2) { for (var img of img2) displayImage(img); }
...
var scrapeCarrib = (urlData) => {
  ...
  if (pID) {
    var img = '<a href="http://click.dtiserv2.com/Direct/8881888-290-184037/moviepages/' + pID[1] + '/index.html" target="_blank"><img src="https://www.caribbeancompr.com/moviepages/' + pID[1] + '/images/l_l.jpg" border="0" height="394" width="700"></a>';
    if (!$('#img').val().match(img)) $('#img').val(img + $('#img').val());
    var video = '<script name="setAffplayer101714_983" id="setAffplayer101714_983" type="text/javascript" src="https://affiliate.dtiserv.com/js/setAffplayerVideo2.js?affid=184037&siteid=290&video=https://smovie.caribbeancompr.com/sample/movies/' 
