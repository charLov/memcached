...
var titleChange = (tNo) => {
  ...
  $('#orgTitle').val($('#title').val().replace(/●|○|♥|☆|★|※|…/g,' '));
};
...
$('#dropdownMinna').on('click', function (event) {
  ...
  var acts = act.match(/.+?：(.+)/);
  if (acts) {
    url = 'http://www.minnano-av.com/' + acts[1];
  }
...
$('.orgButton').on('click', function (event) {
  ...
  if (id) {
    ...
    if (url.length > 5) {
      $('#' + id[1]).append('<div class="orgChild" style="background-color: white;"><iframe src = "' + url + '" style = "width: 100%; height: 1000px;"  allowfullscreen = "true" ></iframe ></div>');
    }
  }
})
...
$('.orgClear').on('click', function (event) {
  ...
  if (input) {
    $('#' + input[1] + 'Input').val('');
    $('#' + input[1] + ' > .orgChild').remove();
  }
//  $('#orgAmazonInput').val('');
})


$('.orgPage').on('click', function (event) {
  var id = event.target.id;
  var pNo = $('#orgNo').val();
  var title = $('#orgTitle').val();
  var tab = id.match(/org(.+?)(No|Title|Url)/);
