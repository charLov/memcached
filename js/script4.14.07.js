...
var test = async function () {
	...
	var res =  await myConfirm('kore?', 'arere?', 'tokucha', 'wawawa', mass);
	console.log('res confirm message after await myCOnfirm',res, mass)
...
$('.clear').click(async function () {
	...
	if ($('#changed').html().indexOf('*') >= 0) {
		await promisePHP('data/' + fileName, 'append', allToText());
		//saveOnClick('append');
	}

	await promisePHP('data/' + fileName, 'move', 'data/complete/' + fileName);
	clearAll();
	setTimeout(function () {
...
var mainDmm2 = async (urlData,retry = 0) => { // DMM 一般　詳細検索
	...
	if (title) {
		if ($('#title').val().length < 1)
			$('#title').val(title[1].replace('/', '　').replace(/[ 　]?（ブルーレイディスク）|[　 ]?【数量限定】|[　 ]?【ベストヒッツ】|[ 　]?【特選アウトレット】|[ 　]?Blu-ray版|[　]?【アウトレット】|[ 　]?（DOD）/g, '').trim());
	} else alert('no title !');
...
var saveOnClick = (mode = 'append',num = 20) => {
	var tree = '';
	if (mode == 'tree') {
		tree = 'tree-';
		mode = 'append';
  ...
  saveTempFile('data/' + fileName.trim(),allToText(tree,ver),mode,num);
    ...
    if (el) {
      ...
      for (var i = 0;i < now.length;i++) {
        ...
        {console.log('Defer line i ',i)}
			...
			if (await myConfirm('check Saved File failture\n\nelement:' + el[1]
        + '\n\nsaved:' + el[2]
        + '\n\nnow : ' + dataNow) == false) break;
    }
  }
