...
var test = async function () {
	var mass = await getFileInfo('basic.html');//filename: fileName, filesize: 'filesize', filetime: 'filetime', thumb: 'thumb'
	console.log(mass.filename,mass)
};
...
//                                        //
//                                        //
  const version = '4.16.20';
//                                        //
//                                        //
////////////////////////////////////////////
/////////// Global var  ///////////////
const baseDataFolder = 'data/';
const completeFolder = 'complete/';
const tempFolder = 'tmp_data/';
...
$(function(){
  ...
  $(window).on("popstate", function (event) {
	...
	////////////  JSTreeを初期化　＆　Load Tree
	// ！！！！！！！！！ とりあえず、非表示　2022/10/25　！！！！！！！！
	//createJSTree();
	//  ！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
  ...
  $('[data-toggle="tooltip"]').tooltip();
	...
	$('#tree-feel').css('height', '15rem');
	$('#tree-append').css('height', '15rem');
	setTrumbowyg('tree-feel', '#ff8');
	setTrumbowyg('tree-append','#f8f');
	setTrumbowyg('notice', '#8ff');
	
	setTrumbowyg('tree-img','#844');
	setTrumbowyg('tree-english','#448');
	setTrumbowyg('tree-prof','#884');
	setTrumbowyg('tree-cmt','#848');
  ...
  getSelect();// change color of select ease
	...
	setTrumbowyg('feel','#ff8',true);
	setTrumbowyg('append','#8ff',true);
	setTrumbowyg('english','#f88',true);
	setTrumbowygHalf('prof','#8f8');
	setTrumbowygHalf('cmt', '#8f8');
	setTrumbowygHalf('img','#88f')
	setTrumbowygHalf('Japanese', '#88f');
	setTrumbowygHalf('translatedText','#8f8');
  ...
  csrTop();
	...
	window.addEventListener("beforeunload", (ev) => {
		...
		if ($('#changed').html().indexOf('*') < 0) return true;
		ev.preventDefault ();
  ...
  setTimeout(function() {
	...
	setTimeout(function () {
		...
		$('#n').click();
		$('#size').val('500');
    console.log('on start idChanged timeout 2000', $('#title').val());
		...
		var fileName = $('#title').val().trim().substring(0, 32).replace(/[:\s|\/]/g, '_') + '.txt';
    readSavedFile(fileName);
		$('#insJapanese').val('挿入単語');
...
var changeImageUrl = (imgUrl, changedSize) => {
	return new Promise(async (resolve, reject) => {
		var imgSize = parseInt(changedSize);
		var wh = [0, 1, 2]; // 0->width, 1->height
		//var imgUrl = $('#img').val().match(/<a href="(.+?)".*?>/);
		var imgInfo = await promisePHP(imgUrl, 'imageInfo');
		console.log(imgInfo)
		var size = imgInfo.match(/width="(\d*?)" height="(\d*?)"/);
		if (!size) reject(imgInfo);

		var rate = parseInt(size[1]) / parseInt(size[2]);
		if (rate < 1) {
			wh[2] = imgSize;
			wh[1] = Math.round(imgSize * rate);
		} else {
			wh[1] = imgSize;
			wh[2] = Math.round(imgSize / rate);
		}
		resolve(wh);
	})
}

$('#nomado').click(async function () {
	//console.log('nomdo  click 321')
  var a = $('#img').val().match(/<a href="(.+?)".*?>/);
  if (!a) return;
  var ah = await myPrompt('画像のリンク先アドレス（未入力ならそのまま）',a[1],'Image Textarea を変更');
	if (!ah) return;
  ...
  if (ah == '') {
	...
	var changedSize = $('#size').val();
	var img = $('#img').val().match(/<img src="(.+?)".*?>/);
	var imgInfo = await changeImageUrl(img[1],changedSize);
	if (imgInfo) {
		await myAlert('width = ' + imgInfo[1], 'height = ' + imgInfo[2]);
		img = '<a href="' + ah + '" target="_blank" rel="noopener noreferrer"><div style="float:left;"><img src="' + img[1] + '" alt="' + $('#title').val() + '" style="max-width:' + changedSize + 'px; max-height:' + changedSize + 'px; width: ' + imgInfo[1] + 'px; height: ' + imgInfo[2] + 'px; float: left; display: flex; justify-content: center; align-items: center; margin: 10px;"></a>';
		$('#img').val(img);
		$('#date').val('');
		$('#prof').val('');
		$('#cmt').val('');
		$('#append').val('');
		await makeCode(true,'moon');
		var txt = $('#code').trumbowyg('html');
		txt = txt.replace(/<p><br><\/p>/g, '\n');
		...
		codeLight();
	} else myAlert(imgInfo,'get image Info')
});
...
var clearAll = () => {
  ...
  $('#p-no').val('');
  $('#img').val('');
  setTextareaValue('append','');
  $('#feel').val('');
  $('#prof').val('');
  $('#cmt').val('');
	$('#url').val('');
	setTextareaValue('english','');
  //$('#english').val('');
  $('#imgURL').val('');
  $('#orgEaseUpInput').val('');
  $('#imgEaseUpInput').val('');
...
var makeCode = (flag, typeofMake = 'white') => { // ture button push false auto make 
	//typeofMake 'moon','altWind'
	...
	return new Promise(async (resolve, reject) => {
		...
		console.log('flag=', flag, 'typeofMake=', typeofMake);
		var pStyle;
		if (typeofMake == 'white') pStyle = DodgerBlueAndDeepSkyBlue;
		...
		var feel = '<!--感想-->' + replaceReg(replaceTag($('#feel').val().trim())) + '<!--感想 ends-->';
		...
		if (flag && (img.length === 0)) {
			if (!await myConfirm('画像がありません', 'コードを作成しますか','継続','キャンセル')) reject();
		...
		eng = eng.replace(/《/g, '{').replace(/》/g, '}')
			...
			txt += '\n\n<div style="border-top-style: double; border-bottom-style: double;"><button type="button" style="border: none;" class="btn btn-outline-primary" onclick="displayAppend(0);">⟨impressions⟩ (in English)<i class="fa-solid fa-angles-down text-warning ml-2" id="engTextIcon"></i></button><div id="engText" style="display: none;"><br>' + eng + '<br></div></div>';
		...
		if (comment.length > 0) {
			txt += '\n\n<!--コメント--><div style="padding: 10px; margin-bottom: 10px; border: 1px     dashed;">' + comment + '</div><!--コメント ends-->';
		}

		if (append.length > 0) {
			var buttonText = 'サンプル動画、画像等';
			if (typeofMake == 'moon') buttonText = '書き抜き・メモ';
			//if (typeofMake == 'moon') {
				append =  '<div style="border-top-style: double; border-bottom-style: double;"><button type="button" style="border: none;" class="btn btn-outline-primary" onclick="displayAppend(1);">' + buttonText + '<i class="fa-solid fa-angles-down text-warning ml-2" id="appendTextIcon"></i></button><div id="appendText" style="display: none;"><br>' + append + '<br></div></div>';
			//}
			txt += '\n<!--追加コード-->' + append + '<!--追加コード ends-->\n';
		}//alert(txt);
		txt += '<!--隠しコード--><br style="clear: both;"><h6 style="color: #888888;font-size: smaller;">[' + hidden + ']</h6><!--隠しコード ends-->';// + '\n\n';
		...
		*/
		...
		var txtPrev = txt;
		var bd = txtPrev.match(/\[nenrei \d{8}\]/g);
		if (bd) {
			txtPrev = txtPrev.replace(/\[nenrei \d{8}\]/g,function(match){
				return genzai(match);
			});
		}

		//	txtPrev = txtPrev.replace('onclick="displayEnglish();', 'onclick="displayEnglishM();');
		var reg = new RegExp('style="max-width:280px;max-height:280px;"', 'g');
		txtPrev = txtPrev.replace(reg,'style="max-width:260px;max-height:260px;"');
		txtPrev = pStyle + txtPrev;
		txtPrev = '<p class="h3 title text-center">'
			+ $('#title').val() + '</p><br>' + txtPrev;
		
		$(txtPrev).appendTo($('#preview_result'));
...
$('#ease').click(async function(){
  ...
  var urlData = $('#url').val();
	...
	if (((dmm != '') && dmm.toUpperCase().indexOf('1PONDO') < 0) &&
		(dmm.toUpperCase().indexOf('CARIB') < 0)  && (dmm.toUpperCase().indexOf('NYOSHIN') < 0) &&
		dmm.match(/[a-z]/) && await myConfirm(dmm,'大文字にしますか？','はい','しません'))
  ...
  if (ease == 'title-r') {
	...
	if (urlData.match('//www.dlsite.com')) {//DLite
		console.log('dlsite')
    dlsite(urlData,true); return; }
...
//https://duga.jp/ppv/orustaksoft-0095/
  ...
  if (urlData.match('.tokyo-hot.com')) { // Tokyo Hot
    ...
    //------------ Gechu ------------
  if (urlData.match('dl.getchu.com')) { // Getchuコスプレ
    getchuPHP(urlData); return;
  }
		//------------ Neowing ---------------
  if (urlData.match('www.neowing.co.jp')) {  // NeoWing 
    ...
    var url = img + '<a href="https://www.neowing.co.jp/aff/click.cgi/PytJTGW7Lok/5651/A221250/product%2F'
      + spd[1] + '" target="_blank" rel="noopener noreferrer">Neowingで詳細を見る<br><br></a>';
    $('#img').val(url);
    return;
	}
    ...
    if (urlData.indexOf('www.dmm.com') > 0) {
      ...
      if (sp) {
        var url = '[sample Movie]\n<video width="100%" controls poster="'
          ...
          + sp[1].substring(0,1) + '/' + sp[1].substring(0,3) + '/' + sp[1] + '/' + sp[1] + '_dmb_w.mp4"><p>ご使用のブラウザでは動画再生に対応していません</p></video>';

				if (append.url >= 0) {
					alert("すでにSample動画があります"); return;
				}
				else {
					setTextareaValue('append',append + url);
				}
    ...
    if (urlData.indexOf("www.dmm.co.jp") < 0) {alert("ＤＭＭのURLではありません。"); return; }
		var video = urlData.indexOf('jp/digital/video');
    ...
    if (pNo) {
      ...
      if (append.indexOf('https://www.dmm.co.jp/litevideo/-/part/=/affi_id=char-001/') < 0) { //動画がないときー追加
        ...
        append += url
          ...
          + pNo[1].substring(0,1) + '/' + pNo[1].substring(0,3) + '/' + pNo[1] + '/' + pNo[1] + '_dmb_w.mp4">'
          + '<p>ご使用のブラウザでは動画再生に対応していません</p></video-->';
				setTextareaValue('append',append);
        aModal('サンプル動画を追加しました','easeUp');
			} else { aModal('サンプルはすでにあります', 'easeUp'); }

			if (video > 0) url = '<a href="https://al.dmm.co.jp/?lurl=https%3A%2F%2Fwww.dmm.co.jp%2Fdigital%2Fvideoa%2F-%2Fdetail%2F%3D%2Fcid%3D' + pNo[1] + '%2F%3Fi3_ref%3Dsearch%26i3_ord%3D2%2F&af_id=char-001&ch=link_tool&ch_id=text" rel="nofollow" target="_blank" rel="noopener noreferrer">Dmmで詳細を見る<br><br></a>';
      else url = '<a href="https://al.dmm.co.jp/?lurl=' + urlData
        + '/&af_id=char-001&ch=link_tool&ch_id=text" rel="nofollow" target="_blank" rel="noopener noreferrer">Dmmで詳細を見る<br><br></a>'
      var img = $('#img').val();
  ...
  if (urlData.indexOf("www.mgstage.com") >= 0) {
    var sp = urlData.split('/');
		...
		if (txt.indexOf('mgstage.com') < 0) {
			txt += '<a href="' + urlData + '?aff=8IMROFDGQICYG35O5PYD6L72F3" target="_blank" rel="noopener noreferrer">MGSで詳細を見る<br><br></a>';
			$('#img').val(txt);
		}
		txt = $('#append').val();
		if (txt.indexOf('mgstage.com') < 0) {
			...
			setTextareaValue('append',txt);
			$('#append').trumbowyg('html', txt);
		}
    return;
  }
  ...
  if (urlData.indexOf('girlsdelta.com') >= 0) {
    ...
    txt += sp[4] + '" target="_blank" rel="noopener noreferrer">GirlsDeltaで詳細を見る</a>';
		$('#img').val(txt);
		return;
  }
    ...
    for (var i in org) {
        ...
        if (res == -1) break;
        var img = $('#img').val();
        var url = '<a href="https://click.dtiserv2.com/Direct/' + des[i] + urlData.substring(res) + '" target="_blank" rel="noopener noreferrer">';
				if (!img.match(url)) {
        ...
        var a = url + '[Original Site (Sample Movie)]</a>';
          ...
          a += '<!--script name="setAffplayer' + str[4] + '" id="setAffplayer'
            ...
            + '/moviepages/' + str[4] + '/images/l_t.jpg&w=700&h=394&url=https://www.' + org[i] + '&id=' + str[4]
					...
					a += '\n<video width="100%" controls poster="https://www.' + org[i] + '/assets/sample/' + str[4] + '/popu/1.jpg" src="http://smovie.' + org[i] + '/sample/movies/' + str[4] + mov[i] + '"><p>ご使用のブラウザでは動画再生に対応していません</p></video>';
        }
        if (!append.match(url)) setTextareaValue('append',append + a);
        break;
      }
    }
	//    return;
...
//  date.setTime(date.getTime() + (60*60*1000));
  ...
  for (var i = 0;i < txt.length / num;i++) {
    $.cookie('codeMaker' + i,txt.substring(i * num, (i + 1)*num),{expires: 14 , path:'/'});
  }
...
var allFromCookie = () => {
  ...
  for (var item of txts) {
    var el = item.match(/<!-- (.+?) -->([\S\s]*)/);
		if (el) {
			if (el[1] == 'category') setCategory(el[2]);
			else setTextareaValue(el[1], el[2]);
			//$('#' + el[1]).val(el[2]);
...
var allToText = (tree = '', ver = 999) => {
  ...
  txt += '<!-- ofile -->' + $('#' + tree + 'ofile').val() + '\t';
	...
	$('input.category').each(function(){
		...
		var cateChecked = $(this).prop('checked');
		txt += '[' + cateName + '](' + cateChecked + ')\n';
	})
	txt += '\t';
...
var setCategory = (txt) => {
	var txts = txt.split('\n')
	for (var category of txts) {
		var cate = category.match(/\[(.+?)\]\((.+?)\)/);
		if (!cate) continue;
		console.log( $('input.category[value=' + cate[1] + ']').val())
		if (cate[2] == 'true') $('input.category[value=' + cate[1] + ']').prop('checked', true);
		else $('input.category[value=' + cate[1] + ']').prop('checked', false);
	}
}

$('#save').click(function(){
  var fileName = $('#title').val().substring(0,32);
	fileName = fileName.replace(/[:\s|\/]/g, '_');
  var date = new Date();
...
var myLoad = () => {
  ...
  var reader = new FileReader();
	...
	console.log('file.webkitRelativePath 1151 =>', file.type, file.size, file.name, url, file.webkitRelativePath)
	console.log(file)
	if (fileName.indexOf(tempPrefix) == 0) {
		fileName = baseDataFolder + tempFolder + fileName;
	} else fileName = baseDataFolder + fileName;
	$('#savedFileName').val(file.name);

  reader.onloadend = function() {
    modalSaved(reader.result,0);
  };
...
$('.change').on('input', (event) => {
	...
	ShowChange(event.target.id);
});

function ShowChange(ele) {
	console.log('1289 ShowChange', ele);
	//makeCode(false);
  ...
  switch (ele) {
	...
	$('#oldDateTime').val(Date.now().toString());
  //setTimeout(function(){
    //makeCode(false);
  //},1000);
	genzai($('#prof').val(), 'n');
...
async function dateClick(){
	...
	var dateDelete = await myConfirm($('#date').val(),'発売日を削除しますか？','削除する','削除しない')
	if (dateDelete) {
		$('#date').val('');
	}
};
...
function getInsert3(selected){
  $('.sub').css('display','none');
  var item = $('#partsItem').val();
	$('.partsCheckbox').css('display', 'none');
	console.log("fff",item)
  switch (item) {
		case 'mp4':
			// 動画の画像ファイル
			var img = $('#append').val().match(/<img.+?src="(.+?)"/);
			console.log($('#append').val(),$('#append').val().match(/<img.+?src="(.+?)"/))
			if (img) {
				selected = img[1];
				console.log(img)
			}
      ...
      $('#partsSubTxt').attr('placeholder','画像ファイル')
        .css('display','block');
			$('.partsCheckbox').css('display', '');
			$('#partsCheckboxText').html('iframe');
      break;
    case 'link' :
      $('#partsTitle').text('リンク先アドレス');
      $('#partsTxt').attr('placeholder','リンク先アドレス');
      $('#partsSubTxt').attr('placeholder','表示テクスト')
        .css('display','block');
			$('.partsCheckbox').css('display', '');
			$('#partsCheckboxText').html('Short');
      break;
    case 'wiki' :
      $('#partsTitle').text('[Wiki]アイテム');
      $('#partsTxt').attr('placeholder','[Wiki(JP)]アイテム');
			$('.partsCheckbox').css('display', '');
			$('#partsCheckboxText').html('US');
      break;
    //case 'wikiUS' :
      //$('#partsTitle').text('[Wiki(US)]アイテム');
      //$('#partsTxt').attr('placeholder','[Wiki(US)]アイテム');
      //break;
    case 'girl' :
  ...
  switch(item) {
    ...
    case 'mp4' : //MP4
      if (txt == '') txt = prompt('リンク先アドレス');
			...
			if ($('#videoIframe').prop('checked')) {
				area.value = start + '<iframe src="' + txt + '" width="100%" height="394">このページはiframe対応ブラウザでご覧ください。</iframe>\n\n' + selected + end;
				break;
			}
    ...
    case 'link' : //リンク
      if (txt == '') txt = prompt('リンク先アドレス');
			...
			if ($('#videoIframe').prop('checked')) {
				...
				area.value = start + '<a href="' + txt + '" target="_blank" rel="noopener noreferrer">' + selected + '</a>' + end;
			}
      break;
    case 'wiki' : //Wiki JP
      if (selected == '') {
        if (txt == '') txt = myPrompt('[wiki]アイテム');
        if ((txt == null) || (txt == "")) return;
      } else txt = selected;
			if ($('#videoIframe').prop('checked')) {
...
var myClick = (e) => {
	//イベント処理
	var key_e = e || window.e; //*3
	var key_shift = (key_e.shiftKey); //*4
	var key_ctrl = (key_e.ctrlKey);
	var key_alt = (key_e.altKey);
	var key_meta = (key_e.metaKey);
	//キーに対応した処理
	var s = "click! ";
	...
	if (key_meta) {
		...
		var area = document.getElementById(e.target.id);
		if (!area) area = document.getElementById($('#str3').text());
		if (!area) return;
		var start = area.value.substring(0,area.selectionStart);
		var selected = area.value.substring(area.selectionStart,area.selectionEnd);
		var end = area.value.substring(area.selectionEnd);
		console.log('click',s,start,selected,end);
		dblClick(e.target.id);
		return;
	}
	//出力
	//console.log('click',s);
...
var dblClick = (id = false) => {
	if (id == false) id = $('#str3').text();
	if (!id) return;
  var area = document.getElementById(id);
  console.log('id = ',id,$('#str3').text(),$('#p-no').val(),area);
  //area.focus();
  //var start = area.value.substring(0,area.selectionStart);
  var selected = area.value.substring(area.selectionStart,area.selectionEnd);
  //var end = area.value.substring(area.selectionEnd);
...
//  txt = txt.replace(/\] +\(/g,'](');
  ...
  if (lin)
    for (i = 0;i < lin.length;i++) {
      ...
      switch (item[2]) {
        case 'L' :
          makeL = '<a href="' + item[1] + '" target="_blank" rel="noopener noreferrer">' + item[1] + '</a>';
        ...
        case 'S' :
          ...
          break;
        case 'M' :
          makeL = '<a href="https://medias.mine.nu/?s=' + item[1] + '" target="_blank" rel="noopener noreferrer">' + item[1] + '</a>';
          txt = txt.replace(lin[i],makeL);
          break;
        case 'U' :
          makeL = item[1] + '<a href="https://en.wikipedia.org/wiki/'
          + item[1] + '" target="_blank" rel="noopener noreferrer">[wiki(US)]</a>';
          txt = txt.replace(lin[i],makeL);
          break;
        case 'W' :
          makeL = item[1] + '<a href="https://ja.wikipedia.org/wiki/'
          + item[1] + '" target="_blank" rel="noopener noreferrer">[wiki(JP)]</a>';
          txt = txt.replace(lin[i],makeL);
          break;
        case 'G' :
          makeL = '<a href="https://nomado.moo.jp/girls/?s=' + item[1] + '" target="_blank" rel="noopener noreferrer"><span class="o2">' + item[1] + '</span></a>';
          txt = txt.replace(lin[i],makeL);
          break;
        case 'GW' :
        case 'WG' :
          makeL = '<a href="https://nomado.moo.jp/girls/?s=' + item[1] + '" target="_blank" rel="noopener noreferrer"><span class="m2">' + item[1] + '</span></a><a href="https://ja.wikipedia.org/wiki/'
            + item[1] + '" target="_blank" rel="noopener noreferrer">[wiki(JP)]</a>';
          txt = txt.replace(lin[i],makeL);
          break;
        default :
      ...
      txt = txt.replace(lin[i],'<a href="' + item[2]
          + '" target="_blank" rel="noopener noreferrer">' + item[1]
          + '</a>');
    }
  return txt;
};
...
function replaceTag(txt){//-[nn]- note を変換
  ...
  for (var i = 0;i < txts.length;i++) {
    ...
    if (line.indexOf('[*]') == 0) {
      rez += '<div class="n">' + line.substring(3) + '</div>\n';
    continue;
    }
    if (line.match(/\[\*\d+?\]/)) {
...
var addTag = (txt) => {
  ...
  for (var line of texts) {
    ...
    var quote = line.slice(3,-4).split(/__|＿＿/g);
		if (quote[0]) {
			var q = quote[0].match(/<span class="quote">(.+?)<\/span>/);
			if (q) quote[0] = quote[0].replace(quote[0], q[1]); 
      txt = txt.replace(quote[0],'<span class="quote">' + quote[0] + '</span>');
    }
    if (quote[1]) {
			var q = quote[1].match(/<span class="note">(.+?)<\/span>/);
			if (q) quote[1] = quote[1].replace(quote[1], q[1]); 
      txt = txt.replace(quote[1],'<span class="note">' + quote[1] + '</span>');
    }
	}
	
	txt = txt.replace(/<span class="formfeed">（FF）<\/span>/g, '（FF）');
	txt = txt.replace(/[\(（][fF][fF][\)）]/g,'<span class="formfeed">（FF）</span>');
	txt = txt.replace(/<span class="linefeed">（LF）<\/span>/g, '（LF）');
	txt = txt.replace(/[\(（][lL][fF][\)）]/g,'<span class="linefeed">（LF）</span>')
...
var addWiki = async (txt, long = null) => {
	if (!txt) return;
	csrWait();
  ...
  str = str.replace(/[ ／]/g,'、');
	...
	for (var i = 0; i < l; i++) {
		csrWait();
		aModal('search ',names[i]);
    var kakko = names[i].match(/(.+?)([（\(].+?[）\)])/);// カッコ役名
		if (kakko) names[i] = kakko[1];
		console.log('1903 names[i] = ',names[i])
    if ((names[i].match(/人目/)) || (names[i] == '') || (names[i].match(/[\[()].*?[\])]/)) || ($('#prof').val().indexOf('[' + names[i] + '](GW)') >= 0)) continue;
    names[i] = names[i].replace('?','');
    var m = names[i].match(/\[.+?\]\((.+?)\)/);

		var sUrl = 'http://www.minnano-av.com/'
    + 'search_result.php?search_scope=actress&search_word=';
		var name = names[i];
		if (m) name = m[1];

		var res = await promisePHP(sUrl + name);
		if (!res) continue;

		var ninzu = res.match(/AV女優の検索結果.+?<span>(\d+?) 件/);
		if (ninzu) {// actless list
			var no = parseInt(ninzu[1]);
			if (no == 0) {await myAlert('Scrape error:no actless:' + name,'1918'); continue;}
			var actless = res.match(/<h2 class="ttl"><a href="(.+?html).+?<td>(\d*?)<\/td>/g);
			if (!actless) {await myAlert('error No actless','1920'); continue;}
...
$('#imgAdd').click(async function(){
  ...
  if ($('#imgURL').val() == '')
    $('#imgURL').val(
  '<img src="https://nomado.moo.jp/girls/wp-content/uploads/2021/05/81fopuIpC2L._AC_SL1500_.jpg" alt="マジメで無口な幼馴染がモジモジ誘惑パンチラで全力アピール チラ見する僕に興奮しちゃって濡れ染みびっちょり！ 八木奈々" height="1500" class="alignnone size-full wp-image-29176" /><img src="https://nomado.moo.jp/girls/wp-content/uploads/2021/05/9mide822pl.jpg" alt="マジメで無口な幼馴染がモジモジ誘惑パンチラで全力アピール チラ見する僕に興奮しちゃって濡れ染みびっちょり！ 八木奈々" width="800" height="439" class="alignnone size-full wp-image-29177">\n<img src="https://nomado.moo.jp/girls/wp-content/uploads/2021/05/51Kom8naX1L._AC_.jpg" alt="その爽やかさ、反則 成宮りか SOD専属AVデビュー" width="352" height="500" class="alignnone size-full wp-image-29158" /><img src="https://nomado.moo.jp/girls/wp-content/uploads/2021/05/81MYsbx1quL._AC_SL1500_-1.jpg" alt="姉の挑発を真に受けた童貞弟がイッてるのに気づかず爆走ピストン　深田えいみ" width="1054" class="alignnone size-full wp-image-29168" />\n<img src="https://nomado.moo.jp/girls/wp-content/uploads/2021/05/f6191719fff91f4f7382ef5a89ccbacd.jpg" alt="「頭の中がおち○ちんの事でいっぱいなんです…」 竹内乃愛 思春期の少女がハマるおち○ぽ研究" width="800" height="537" class="alignnone size-full wp-image-29151" />\n<img src="https://nomado.moo.jp/girls/wp-content/uploads/2021/05/DHgLx2wV0AAUwBm.jpg" alt="「頭の中がおち○ちんの事でいっぱいなんです…」 竹内乃愛 思春期の少女がハマるおち○ぽ研究" class="alignnone size-full wp-image-29153" />'
  );


	var img = $('#imgURL').val();
	img = img.replace(/src="\/\/img.dlsite/g, 'src="https://img.dlsite');
	var ap = img.match(/https?:.+?\.[jpen]{2,3}g/g);
	var jpg = img.match(/\.jpe?g$/);
	var png = img.match(/\.png$/);
	if (ap) console.log('image =>', ap)
  ...
  var clone = ele.cloneNode( false ); //ガワだけ複製して…
	...
	if (jpg || png) {
		...
		for (var i = 0; i < ap.length; i++) {
			...
			var res =  addNode(ap[i], true, ap.length, count);
			if (res) count++;
			else aModal('make imageList error', 'imgNo=' + count);
		}
	}
  $('#imageModal').modal();
});
...
var addNode = function(ap,b,max,num) {//ap=<img src=... b=check true 
  ...
  var image = ap.match(/(https?:.+?\.[jpen]{2,3}g)/);
  if (!image) return false;
  
  if ($('#imageList').html().indexOf(image[1]) >= 0) return false;
  ...
  if (image) {
    ...
    src.src = image[1];
    src.className = 'img-rounded img-thumb mx-1';
    src.height = 100;
    src.alt = alt;
    img.appendChild(src);
    el.appendChild(img);
...
var mainImage = () => {
	var alt = $('#title').val();
	console.log('2258 ',alt)
	if (!alt || (alt.length) < 2) alt = 'alt="snap shot"';
	else alt = 'alt="' + alt + '"';
  for (var i = 0;;i++) {
    ...
    if (num == '0') {
      ...
      var item = $('#numOld' + i).val();
			var src = item.match(/(https?:.+?\.[jpen]{2,3}g)/);
      ...
      var orgImage = img.match(/(https?:.+?\.[jpen]{2,3}g)/);
      ...
      if (orgImage) {
				...
				console.log('maiImage match orgImage')
      } else if (ahref) {
        $('#img').val(img.substring(0,img.indexOf(ahref[0]) + ahref[0].length)
					+ altImage + img.substring(img.indexOf(ahref[0]) + ahref[0].length));
				console.log('mainImage match <a href')
      } else {
				$('#img').val('<a href="' + src[1] + '" data-lightbox="example-1">' + altImage + '</a>' + img);
				console.log('mainimage match nothing')
      }
      return;
    }
  }
        ...
        if ((width) && (height)) {
          if (parseInt(width[1]) > parseInt(height[1])) {
            ...
            height[1] = (size -20);
				...
				sub += snapImage(src[1]);
				console.log('sub Image',src[1])
        break;
      }
    }
  }
  if ((app != '') && (sub != '')) app += '\n\n';
  setTextareaValue('append',app + sub);
};
...
*/
var asDmm = async (pNo,urlData) => {  //　商品IDだけで、情報を作成
  ...
  if (no) {
		...
		//https://cc3001.dmm.co.jp/litevideo/freepv/5/561/5619bbag00009/5619bbag00009_sm_w.mp4
		//<iframe width="720" height="480" src="https://www.dmm.co.jp/litevideo/-/part/=/affi_id=char-001/cid=h_579bbag009r/size=720_480/" scrolling="no" frameborder="0" allowfullscreen></iframe>
		if ($('#append').val().indexOf('https://www.dmm.co.jp/litevideo/-/part/=/affi_id=char-001/') < 0) {
			url += '\n<!--video width="100%" controls poster="https://pics.dmm.co.jp/mono/movie/adult/'
				+ pNo + '/' + pNo + 'pl.jpg"  src="https://cc3001.dmm.co.jp/litevideo/freepv/'
				+ pNo.substring(0, 1) + '/' + pNo.substring(0, 3) + '/' + pNo + '/' + pNo + '_dmb_w.mp4"><p>ご使用のブラウザでは動画再生に対応していません</p></video-->';
		}
  }

  var img = '<img src="https://pics.dmm.co.jp/mono/movie/adult/' + pNo + '/' + pNo + 'pl.jpg">';
	//https://pics.dmm.co.jp/digital/video/5619bbag00009/5619bbag00009jp-1.jpg
	//https://pics.dmm.co.jp/digital/video/h_346rebdb00087/h_346rebdb00087jp-1.jpg
	//https://pics.dmm.co.jp/digital/video/h_346rebdb00087/h_346rebdb00087jp-1.jpg
	//https://pics.dmm.co.jp/digital/video/5619bbag00009/5619bbag00009jp-2.jpg
	//https://pics.dmm.co.jp/digital/video/5619bbag00009/5619bbag00009pl.jpg
	//https://pics.dmm.co.jp/digital/video/huntb00304/huntb00304jp-1.jpg
	// sample image
	var sampleImage = '<img src="https://pics.dmm.co.jp/mono/movie/adult/' + pNo + '/' + pNo + 'pl.jpg">';
	if (no) {
		var dmmNo = no[1] + no[2] + ('0000' + no[3]).slice(-9 + no[1].length + no[2].length);
		for (var count = 1; ; count++) {
			if (count > 30) break;
			var imgUrl = 'https://pics.dmm.co.jp/digital/video/' + dmmNo + '/' + dmmNo + 'jp-' + count + '.jpg';
			var imgExist = await promisePHP(imgUrl, 'isfile');
			console.log('imag exists ', imgExist);
			if (imgExist.indexOf('が存在します') < 0) break;
			sampleImage += '<img src="' + imgUrl + '">';
		}
	}
	$('#imgURL').val(sampleImage);

  var p = $('#append').val();
  if (!p.match(url)) {
    if (p.length > 1) url = p + '\n\n\n' + url;
    setTextareaValue('append',url);
  }

  img = '<a href="https://al.dmm.co.jp/?lurl=' + urlData + '&af_id=char-001&ch=link_tool&ch_id=link" target="_blank" rel="noopener noreferrer">Dmmで詳細を見る<br><br></a>';
  p = $('#img').val();
    ...
    fetch(urlx,{method: "GET",}).then(response => response.text()).then(text => {
      ...
      if (title) {
        if ($('#title').val().length < 1)
					$('#title').val(title[1].replace(/[ 　]?（ブルーレイディスク）|[　 ]?【数量限定】|[　 ]?【ベストヒッツ】|[ 　]?【特選アウトレット】|[ 　]?Blu-ray版|[　]?【アウトレット】|[ 　]?（DOD）/g, '').normalize( "NFC" ));
      } else aModal('no title !','mainDmm');

			var cmt = json.replace(/\r|\n|\t/g, '').match(/<p class="mg-b20">(.+?)<\/p>/);
			if (cmt) {
				//for (var i = 0; ;i++) {
					var cmtLink = cmt[1].match(/<a href="(.+?)"/);
					if (cmtLink) {
						cmt[1] = cmt[1].replace(cmtLink[0], '<a href="https://al.dmm.com/?lurl=' + cmtLink[1] + '&af_id=char-001&ch=link_tool&ch_id=link"');
					} //else break;
				//	if (i > 10) break;
				//}
				if (($('#cmt').val().length < 1)) $('#cmt').val(cmt[1].trim()); else alert('no comment !');//発売日：</td>
			}
			var date = json.match(/発売日：.+?\s.+?(\d{4}\/\d{2}\/\d{2})/);
			if ($('#date').val() == '') {
				if (date) $('#date').val(date[1]); else aModal('no date !','mainDmm');
			}
      var prof = $('#prof').val();
      ...
      if ((act) && (prof.indexOf('出演：') == -1)) {
        ...
        console.log(act[0]);
        if (acts) {
          var actless = acts[0].substring(1,acts[0].length - 3);
          perform = '出演：' + actless;
          if (title) {
          ...
          for (var i = 1;i < acts.length;i++) {
            ...
            perform += '、' + acts[i].substring(1,acts[i].length - 3);
            console.log(perform);
          }
        }
      }
...
var mainDmm2 = async (urlData,retry = 0) => { // DMM 一般　詳細検索
	...
	if (title) {
		if ($('#title').val().length < 1)
			$('#title').val(title[1].replace('/', '　').replace(/[ 　]?（ブルーレイディスク）|[　 ]?【数量限定】|[　 ]?【ベストヒッツ】|[ 　]?【特選アウトレット】|[ 　]?Blu-ray版|[　]?【アウトレット】|[ 　]?（DOD）/g, '').trim().normalize( "NFC" ));
	} else aModal('no title !','mainDmm2');

	var cmt = txt.match(/<p class="mg-b20">([\s\S]+?)<div/);
	if ((cmt) && ($('#cmt').val().length < 1)) $('#cmt').val(cmt[1].trim()); else await myAlert('no comment !');//発売日：</td>
//<td width="100%">2021/02/25</td>

	var date = txt.match(/発売日：[\s\S]+?(\d{4}\/\d{2}\/\d{2})/);
	if ((date) && ($('#date').val() == '')) $('#date').val(date[1].trim());
	...
	if ((act) && (prof.indexOf('出演：') == -1)) {
		...
		if (acts) {
			...
			if (title) {
				...
				var acter = title[1].match(new RegExp(' ' + actless + '$'));
				if (acter) $('#title').val(
					$('#title').val().replace(acter, '　' + actless).replace(/[ 　]?（ブルーレイディスク）|[　 ]?【数量限定】|[　 ]?【ベストヒッツ】|[ 　]?【特選アウトレット】|[ 　]?Blu-ray版|[　]?【アウトレット】|[ 　]?（DOD）/g, '').normalize("NFC")
				);
			}
			for (var i = 1;i < acts.length;i++) {
				if (acts[i].length < 6) continue;
  ...
  setTimeout(function() {
    ...
    fetch(urlx,{method: "GET",}).then(response => response.text()).then(text => {
      ...
      // https://www.dmm.com/mono/dvd/-/detail/=/cid=n_701tsbs81025/
			console.log('scrapeDmm2901.html');
      saveToFile(json,'scrapeDmm2901.html');
...
var csrToggle = () => {
	...
	//csr = csr + ' !important';
	for (var ele of eles) {
		$(ele).toggleClass('wait');
		$(ele + ':hover').toggleClass('wait');
	}
...
var aModal = (txt,ele = 'codeMaker',t = 10000) => {
  ...
  txt = txt.slice(-24);
	...
	// txt の内容で$('#changed').html('*');を表示するかどうか
	var notChanges = ['保存'];
	var changes = ['作成', 'scrape']
	for (var change of changes) {
		if (txt.indexOf(change) >= 0) {
			$('#changed').html('*' + ele).css('color','#fbb');
			$('#oldDateTime').val(Date.now().toString());
			waitIcon();
		}
	}
	//for (var notChange of notChanges) {
	//	if (notChange == txt) $('#changed').html('');
	//}
...
$('.nav-pills a').on('hidden.bs.tab', function (event) {
  ...
  $('#p' + pill[0]).css('color','#8c8');
	$('.trumbowyg-editor-visible').removeClass('trumbowyg-fullscreen');
	$('.trumbowyg-fullscreen-placeholder').css('display', 'none');
});
...
/*    if (!s[0].match(/\[.+?\]\(.+?\)/)) { // [](**)がないとき
  ...
  for (i = 1;i < s.length;i++) {
    ...
    ss = s[i].match(/サイズ[\s\S]*?T\D?(\d{1,3})\D*?B\D?(\d{1,3})\D*?W\D?(\d{1,3})\D*?H\D?(\d{1,3})/);
    if (ss) { // サイズ　身長あり
      a += 'サイズ T' + ss[1] + ' / B' + ss[2] + ' / W' + ss[3] + ' / H' + ss[4];
      continue; 
		}
		
    ss = s[i].match(/サイズ[\s\S]*?B\D?(\d{1,3})\D*?W\D?(\d{1,3})\D*?H\D?(\d{1,3})/);
    if (ss) { // ザイズ　身長なし
      a += 'サイズ B' + ss[1] + ' / W' + ss[2] + ' / H' + ss[3];
      continue; 
		}
...
var all = () => {//Allcinema New
  ...
  while (i < line.length -1){
    ...
    if (line[i + 1].indexOf("\t") == 0) {
      var cara = line[i + 1].substring(1).length;
      if (cara > 0) {
      ...
      credit += line[i] + line[i + 1].substring(1,line[i + 1].length) + "　"; i+= 2;
    } else {
      credit += line[i] + "\n"; i++;
    }
  }
  ...
  if (confirm('タイトル、メディア、クレジット' + '\n\n' + '書き換えますか？')) {
    $('#title').val(title.normalize( "NFC" ));
    setTextareaValue('append',media);
    $('#prof').val(credit);
    $('#s').click();
  }
}
...
var wiki = function(){
  ...
  var rez = '';
///// s[0] kana s[1] kanji
	var kanaMoji = kana2roma(s[0]).split(/\s/g);
	var kanaUpper = '';
	for (var i = 0; i < kanaMoji.length; i++) {
		kanaUpper += ' ' + kanaMoji[i].substring(0,1).toUpperCase() + kanaMoji[i].substring(1);
	}
...
//    alert('<<' + s[i] + '>><<    2003年 - 現在>>活動内容 ')
    ...
    if ((name) && (i < s.length -1) && (s[i + 1].match(/.+? .+?プロフィール/))) {
      ...
      for (var j = 0;j < kana.length;j++) {
        ...
        he = he.substring(0,1).toUpperCase() + he.substring(1);
        roma += he;
      }
      ...
      case '出身地':
        ...
        if (sb.length == 1)
          rez += ss[0] + ' ' + sb[0] + '\n\n';
					...
					rez += ss[0] + ' ' + sb[1] + '\n\n';
				rez = rez.replace('日本の旗 ', '').replace('日本 ', '');
        break;
      case '生誕':
      case '誕生日':
      case '生年月日' :
...
//    alert('i=' + i + ' ' + kana.substr(i,1));
    if (i < kana.length - 1) {
      for (var j = 0;j < hebon2.length;j++) {
        if (hebon2[j][0] == kana.substring(i,i + 2)) {
          henkan = hebon2[j][1]; i++;
          break;
        }
      }
    ...
    if (henkan == '') {
      for (var j = 0;j < hebon.length;j++) {
        if (hebon[j][0] == kana.substring(i,i + 1)) {
          henkan = hebon[j][1];
        }
      }
    }
    if (henkan == '') roma += kana.substring(i,i + 1);
    else roma += henkan;
  }
...
var setTrumbowyg = (area, color = '#cc6', toggle = true) => {
	...
	$('#' + area).trumbowyg({
		...
		btnsDef: {
			...
			moreForm: {
				dropdown: ['blockquote','p',  'h5', 'h4', 'h3', 'h2', 'h1', 'h6','horizontalRule'],
				title: 'フォーマット',
				ico: 'p'
			},
			myList: {
			...
			myLink: {
				dropdown: [ 'createLink', 'unlink', 'insertImage'],
				title: 'リンク',
				ico: 'link'
			},
			para: {
			...
			personal: {
				dropdown: ['clear','myNote','myAdjust','count','myUpload','myClose'],
				title: '独自関数',
				ico: 'person'
			},
        ...
        key: 'S',
			...
			count: {
				text: '<span id="trumbowyg-count-' + area + '">文字数</span>',
				ico: 'my-list'
			},
			clear: {
					fn: function() {
	...
	//　フォーカスを失ったとき
	.on('tbwblur', function(){
		console.log('tbwblur wEditor !!!',area,toggle);
		/*if (area) {
			var targetTxt = $('#' + area).val();
			var trumbowygTxt = $('#' + area).trumbowyg('html');
			if (targetTxt != trumbowygTxt) {
				$('#' + area).val(trumbowygTxt);
			}
		}*/
	})
  //　変更されたとき
	.on('tbwchange', function () {
		if (area == 'code') return;
		var count = $('#' + area).trumbowyg('html').length;
		...
		$('#trumbowyg-count-' + area).html(count.toLocaleString() + '字');
	})
	//////// fullscreen //////////		
...
//		})
	...
	if (toggle) $('#' + area).trumbowyg('toggle');

	setTimeout(() => {
		$('#' + area).trumbowyg('html',txt);
	}, 500);

}


var setTrumbowygHalf = (area, color = '#cc6', toggle = true) => {
	$('#' + area).toggleClass('trumbowyg-dark');
  ...
  //　変更されたとき
	.on('tbwchange', function () {
		if ((area == 'code') || (area == 'translatedText') || (area == 'Japanese'))
			return;
		$('#changed').css('color', color);
		var change = 't';
		if (area) change = area//.substr(0, 1);
...
var titleChange = async (tNo) => {
  var titles = ['title','title2','title3'];
  var title = $('#' + tNo).val()
		.replace(/[ 　]?（ブルーレイディスク）|[　 ]?【数量限定】|[　 ]?【ベストヒッツ】|[ 　]?【特選アウトレット】|[ 　]?Blu-ray版|[　]?【アウトレット】|[ 　]?（DOD）/g, '')
		.normalize();
	$('#' + tNo).val(title);
	if (title.length == 0) document.title = 'codeMaker';
	else document.title = title;
	console.log('4626 title change ',title)
	//$('#changed').html('');
	if (!title || (title == '')) {
		return;
	}
  ...
  reader.onload = function(){
    ...
    for (var line of txts) {
      var ele = line.match(/<!-- (.+?) -->([\S\s]*)/);
			if (ele) {
				if (ele[1] == 'category') setCategory(ele[2]);
				else setTextareaValue(ele[1], ele[2]);
				//$('#' + ele[1]).val(ele[2]);
				//$('#' + ele[1]).trumbowyg('html', ele[2]);
...
var hotPHP = async (filename) => {
  ...
  if ((actless) && (!act.match('不明'))) {
    $('#title').val(($('#title').val() + '　' + act).normalize( "NFC" ));
...
var getchuPHP = async (filename) => {
	var pno = filename.match(/item(\d+)/);
	if (!pno) {
		aModal('no item no', 'getchuPHP');
		return;
	}
	$('#img').val(
		$('#img').val() + '<a href="http://dl.getchu.com/r.php?aff=115009-01-00&t=2&gcd=' + pno[1] + '">ゲッチュで詳細を見る<br><br></a>'
	);
	csrWait();
  var res = await promisePHP(filename);
  if (!res) {
    aModal('error Scrape', 'getchuPHP');
    return;
  }
	var title = res.match(/<meta property="og:title" content="([\s\S]+?)"/);
	if (title) $('#title').val(title[1].normalize( "NFC" ));
...
//  var filename = 'https://www.gekiyasu-dvdshop.jp/products/list.php?mode=search&name=' + pNo;
  ...
  if ((title) && ($('#title').val() == '')) {
    ...
    if (last != -1) txt = txt.slice(0,last) + '　' + txt.slice(last + 1);
		$('#title').val(txt.replace(/[ 　]?（ブルーレイディスク）|[　 ]?【数量限定】|[　 ]?【ベストヒッツ】|[ 　]?【特選アウトレット】|[ 　]?Blu-ray版|[　]?【アウトレット】|[ 　]?（DOD）/g, '').normalize( "NFC" ));
  }
  ...
  if (!imgVal.match('<a href="http://www.gekiyasu-dvdshop.jp'))
    $('#img').val(imgVal + '<a href="http://www.gekiyasu-dvdshop.jp/affiliate/link.php?'
      ...
      + filename + '" target="_blank" rel="noopener noreferrer">激安DVDショップで詳細を見る<br><br></a>');
...
var testScrapePHP = async (filename,save = 'none') => {
  ...
  var url = "saveTemp4.14.03.php"//?mode=write?filename=" + fileName;
	var ret = '';
	var res;
...
/* delete when work good
var minnaPHP = async (res,name, flag = true) => {
  var sUrl = 'http://www.minnano-av.com/'
...
$('.savePHP').on('click',async function(){
  await saveOnClick('overwrite');
})
$('.appendPHP').on('click',async function(){
  await saveOnClick('append');
})

var saveOnClick = (mode = 'append', numStr = '20') => {//
	// tree saveButton numStr==tree,mode==overwrite auto numStr==#changed.val
	// numStr = String
	savingIcon();
	return new Promise(async (resolve, reject) => {
		...
		var tree = '';
		if ((numStr == 'tree') || (changed.indexOf('tree') >= 0)) {
			tree = 'tree-';
			numStr = '000';
			//mode = 'append';
		}
		// tree 決定
		var fileName = $('#' + tree + 'title').val().substring(0,32).trim() + '.txt';
		fileName = fileName.replace(/[:\s|\/]/g,'_');
		if (fileName == '.txt') fileName = 'untitled.txt';
		// fileName 決定

		var num = parseInt(numStr);
		console.log('is append ? ', numStr);
		if ((numStr == '000') || isNaN(num)) {
			// numStr 000 or not numberString or Tree
			fileName = tempFolder + tempPrefix + fileName;
			num = 999;
			mode = 'overwrite';
			console.log('save on click 5231 mode=',mode)
			//if (numStr != 'temp') await myAlert('saveTempFile not temp num=' + numStr + '\n\nfilename =' + fileName);
		}
		//var date = new Date();
		var dataFileName = baseDataFolder + fileName.trim();
		console.log(dataFileName);

		if ((mode == 'append') || (mode == 'clear')) {
			var maxByte = parseInt($('#maxByte').val());
			var maxBackup = parseInt($('#maxBackup').val());
			var byte = await promisePHP(dataFileName, 'filesize');
			var myByte = byte.match(/int\((\d*)/);
			if (myByte) {
				var filesize = parseInt(myByte[1]);
				if (filesize > maxByte * 1024 * 1024) {
					if (await myConfirm('ファイルサイズが' + maxByte.toLocaleString() +'MByteを越えました。\n\nfilesiza=' + filesize.toLocaleString() + 'bytes', 'バックアップ保存しますか？', 'バックアップ保存', '上書き保存')) {
						var his = 0;
						var searchFileName = completeFolder + fileName.slice(0, -4) + '(' + his + ').txt';
						var isFileName = await promisePHP(searchFileName, 'thumbnail');
						while (isFileName.indexOf('が存在します') >= 0) {
							his++;
							searchFileName = baseDataFolder + completeFolder + fileName + '(' + his + ').txt';
							isFileName = await promisePHP(searchFileName, 'thumbnail');
							if (his > maxBackup) break;
						}
						aModal('現在のバックアップ数', his.toString());
						await promisePHP(baseDataFolder + fileName, 'move', searchFileName);
					}
					mode = 'overwrite';
				}
			}
		...
		var ver = '000';
		var thumbnail = await promisePHP(dataFileName, 'thumbnail');
		console.log(thumbnail);
		if ((thumbnail) && (thumbnail.indexOf('が存在します') >= 0)) {
			var thumbVer = thumbnail.match(/\sver\.(\d{3})\s/);
			if (thumbVer)
				ver = ('000' + (parseInt(thumbVer[1]) + 1).toString()).slice(-3);
		}
		var res;
		if (mode == 'clear') res = await promisePHP(dataFileName, 'append', allToText(tree, ver), ver);
		else {
			res = await promisePHP(dataFileName, mode, allToText(tree, ver), ver);
		}
		if (res) {
			var date = new Date();
			...
			if (resByteStr) {
				...
				await myAlert(res,'write file error !!');
				syncedIcon();
				return reject(false);
			}
			...
			if (num == 999) aModal('仮保存しました',time);
				...
				await promisePHP(baseDataFolder + tempFolder + tempPrefix + fileName, 'overwrite', allToText(tree,ver),ver);
				aModal('追加保存しました ver.' + ver, time);
			}
			else if (mode == 'move') aModal('移動', res.match(/(移動.+?)\n/)[1].slice(-16));
			else if (mode == 'delete') aModal('削除', res.match(/(削除.+?)\n/)[1].slice(-16));
			else if (mode == 'clear') {
				res = await promisePHP(baseDataFolder + fileName, 'move', baseDataFolder + completeFolder + fileName);
				await promisePHP(baseDataFolder + tempFolder + tempPrefix + fileName, 'delete');
				aModal('クリアー＆移動', res.match(/(移動.+?)\n/)[1].slice(-16));
			}
			else aModal('保存しました:' + mode, time);
		} else {
			console.log('post failture',xmlhttp.responseText);
			await myAlert(xmlhttp.responseText,'Post failture');
			reject(xmlhttp.responseText);
		}
		syncedIcon();

		var oldTitle = $('#oldTitle').val();
		var title = $('#title').val().substring(0, 32).trim().replace(/[:\s|\/]/g, '_') + '.txt';
		$('#oldTitle').val(title);
		if ((oldTitle == '.txt') || (title == '.txt') || (oldTitle == '') || (title == '')) {
			console.log('5542 titlechange title,oldefilename', title, oldTitle, (title == oldTitle))
			reject(false);
		} else {
			console.log('5545 ', title, oldTitle, (title == oldTitle))
			if ((title != oldTitle)) {
				//if ((oldTitle == '') || (title == '')) return;
				console.log('delete old filename begin')
				var filename = baseDataFolder + oldTitle;
				var res = await promisePHP(filename, 'thumbnail');
				if (res.indexOf('が存在します') >= 0) {
					var del = await myConfirm('Old filename ' + res + '\n\n title oldTitle 比較\n\n\n新ファイル：' + title + '\n\n旧ファイル：' + oldTitle + '\n\n比較結果：' + (title == oldTitle), '古い保存ファイルを削除しますか', '削除', 'いいえ')
					if (del) {
						res = await promisePHP(filename, 'delete');
						console.log('5523 delete old normal file ', filename, res)
					}
				}

				filename = baseDataFolder + tempFolder + tempPrefix + oldTitle;
				var res = await promisePHP(filename, 'thumbnail');
				if (res.indexOf('が存在します') >= 0) {
					var del = await myConfirm('Old temp filename ' + res + '\n\n title oldTitle 比較\n\n\n新ファイル：' + title + '\n\n旧ファイル：' + oldTitle + '\n\n比較結果：' + (title == oldTitle), '古いtempファイルを削除しますか', '削除', 'いいえ')
					if (del) {
						res = await promisePHP(filename, 'delete');
						console.log('5532 delete normal file ', filename, res)
					}
				}
				console.log('5567 titlechange title,oldefilename', title, oldTitle, (title == oldTitle))
			}
			//$('#oldTitle').val(title);
...
$('#fontSize').on('change', () => {
	...
	/*var eles = ['textarea', 'input', '.trumbowyg-textarea', '.trumbowyg-editor', '.trumbowyg-editor p'];
	for (var ele of eles) {
		if (size == 'medium') {
			$(ele).css('font-size', '');
			continue;
		}
		var defaultStyle = $(ele).attr('style');
		console.log('5304 defaultstyle ', ele, ' test ', ' style ', defaultStyle)
		if (defaultStyle) {
			defaultStyle = defaultStyle.replace(/font-size.+?;/, '') + 'font-size: ' + size + ' !important;';
			$(ele).css('cssText', defaultStyle);
		} else {
			console.log('5310 defaultstyle ', ele, ' test ', ' style none')
			$(ele).css('cssText', 'font-size: ' + size + ' !important;');
		}
	}
	//array.forEach(element => {
	//});
	//$('textarea,input,.trumbowyg-textarea,.trumbowyg-editor,.trumbowyg-editor p').css('font-seize', size);
	//font-size: xx-large !important;
	//line-height: 1.5 !important;
	*/
})

$('#doSetting').on('click',function(){
  //設定する
})


$('#listTempFile').on('click', () => {
	getTempList(baseDataFolder + tempFolder);
  $('#dir').text(baseDataFolder + tempFolder);
})

$('.listPHP').on('click',function(){
  getTempList(baseDataFolder);
  $('#dir').text(baseDataFolder);
})

var fetchTempFile = (fileName,mode = 'none') => {
	...
	//        console.log(txt.length);
			...
			if (mode == 'none') {
				...
				csrAuto();
				resolve(rText);
			} else {//　fileList click mode=inputNumber
				$('#divList' + mode).toggleClass('hideTextarea');
				$('#divList' + mode).toggleClass('showTextarea');
				//var hideTextarea = $('#areaList' + mode).css('display');
				if ($('#divList' + mode).hasClass('showTextarea')) {
					//for (var line of txt) {
						//          console.log(line);
...
var listToModal = (txt) => {
	//console.log('post success = ', 'list',txt);
	var txts = txt.split(/\n/g);
	//console.log('txts = ', '5445',txts);
	
	txts.sort((a, b) => a.localeCompare(b), 'ja'); // sort LIST
	//  console.log(txts);
  ...
  for (var i = 0;i < txts.length;i++) {
    ...
    //$('#fileList').append('<hr>',{class: 'm-0 p-0'});

		var fTime = '';
		var fileSize = '';
		var fileTime = filename.match(/^(.+)\t(.+?)\t(.*?)$/);
		if (fileTime) {
			filename = fileTime[1];
			fTime = fileTime[2];
			fileSize = parseInt(fileTime[3]).toLocaleString() + 'bytes';
		}
      ...
      $('#fileList').append(
        $('<div>',{class: 'clearfix my-1'})
          .append($('<input>',{
            class: 'col-sm-11 nowrap p-0 m-0 dark text-warning',
						style: 'border: none;',
						readonly: 'readonly',
      ...
      continue;
		...
		$('#fileList').append(
			...
			.append($('<input>', {
				...
				style: 'border: none;',
				//                  onclick: 'readFeel("' + file + ',' + no + '")',
				value: filename + '   ' + fTime + ' ' + fileSize,
				//onblur: 'closeFeelP("areaList' + i + '")',
				ondblclick: 'showThumbnail("' + dir + filename + '","' + i + '")',
			}))
			.append($('<button>', {
				class: 'col-sm-1 btn btn-sm float-right btn-outline-danger',
				//          text: '<i class="fas fa-pen-nib"></i>',
			...
			.append($('<button>', {
				...
				//          text: '<i class="fas fa-pen-nib"></i>',
				id: 'completeFileButton' + i,
				onclick: 'moveToComplete("' + filename + '")',
			}).append($('<i class="fas fa-clipboard-check"></i>')))
			.append($('<button>', {
				class: 'col-sm-1 btn btn-sm float-right btn-outline-success',
				//          text: '<i class="fas fa-pen-nib"></i>',
				id: 'editFileButton' + i,
				onclick: 'fetchTempFile("' + dir + filename + '","modal")',
			}).append($('<i class="fas fa-pen-nib"></i>')))
			.append($('<br>'))

			.append(
				$('<div>', {
					class: 'col-sm-12 trumbowyg-dark p-0 m-0 divList hideTextarea',
					style: 'width: 100% color: red !important;',
					id: 'divList' + i,
				})
				.append($('<textarea>', {
					class: 'form-control wEditor dark',
					row: '4',
					text: i,
					//onblur: 'closeFeelP("divList' + i + '")',
		...
		$('#editFileButton' + i).attr('data-dismiss', 'modal');
		setTrumbowygHalf('areaList' + i, '#8f8');

  }
  $('#modal1').modal();
}

var showThumbnail = async (fileName, i) => {
	//$('.divList').removeClass('showTextarea');
	$('.divList').addClass('hideTextarea');
	$('#divList' + i).removeClass('hideTextarea');
	//var hideTextarea = $('#divList' + i).css('display');
	if ($('#divList' + i).hasClass('hideTextarea') == false) {
		res = await promisePHP(fileName, 'read'); 
		if (!res) {
			myAlert(fileName, '読み込みエラー');
			return;
		}
		var startPoint = res.indexOf('<!-- data start');
		if (startPoint < 0) {
			myAlert(res, 'データがありません');
			return;
		}
		res = res.substring(startPoint);
		setTextareaValue('areaList' + i,res);
		var area = document.getElementById('areaList' + i);
    area.focus();
	}
}
...
var moveToComplete = async (fileName,flag = true) => {
  var xmlhttp = new XMLHttpRequest();
  var dir = $('#dir').text();
  var txt = baseDataFolder + completeFolder + fileName;
  fileName = dir + fileName;
  aModal(fileName + ' を移動しました',txt);

	var res = await promisePHP(fileName, 'move',txt);
	if (res) {
		console.log('post success moveToComplete txt.length = ', res.length,res);
		if (flag) getTempList(dir);
	} else {
		console.log('post failture moveToComplete');
		await myAlert(fileName + 'move'+ txt,'Post failture moveToComplete');
	}
}
...
var deleteFeelP = async (fullName,flag = true) => {
  var dir = $('#dir').text();
	...
	if (txt) {
		...
		if (txt.indexOf('削除できない！') >= 0) myAlert(txt);
		if (flag) getTempList(dir);
	} else {
...
var closeFeelP = (ele) => {
	$('#' + ele).addClass('hideTextarea');
	$('#' + ele).removeClass('showTextarea');
} 

var autoSave = (stop = false) => {
	minute = parseInt($('#timeSpan').val());
	char = parseInt($('#charSpan').val());
  var count = 1;
  var textLengthOld = allToText().length;
	var textSaved = allToText();
  //var date = new Date.now();
	var timerNo = parseInt($('#autoSaveNumber').val());
	if (!isNaN(timerNo)) {
  ...
  if (stop) {
	...
	var autoS = async () => {//　文字数
    var textLengthNow = textSaved.length;
    if ((textLengthNow - textLengthOld > char) || (textLengthOld - textLengthNow > char)) {
      console.log('autosave count = ' + count,textLengthNow,textLengthOld,char);
      await saveOnClick('append',$('#changed').html());
      textLengthOld = textLengthNow;
		} else {
			var textNow = allToText();
			var changeEle = $('#changed').html();
			if (textNow.length == textSaved.length) {
				if (changeEle.indexOf('*') >= 0)
					$('#changed').html('NS').css('color', '#403');
				else {
					syncedIcon();
					return;
				}
			}
			if (changeEle == '') changeEle = 'autoSave';
			console.log('texts differ time 5816',changeEle, (textNow == textSaved))
			textSaved = textNow;
			console.log('5931 before await')
			var awaitSave = await saveOnClick('overwrite', changeEle);
			return (awaitSave);
    }
		//syncedIcon();
  }
	var interval = setInterval(autoS, minute * 1000);

	$('#autoSaveNumber').val(interval);

	console.log('autoSave Started timeSpan ', minute);
}


var checkSave = async (saved) => {
  ...
  for (var ele of saves) {
    ...
    if (el) {
      ...
      for (var i = 0;i < now.length;i++) {
        if (now[i] != sav[i])
        {console.log('Defer line i ',i)}
      }
			if (await myConfirm(el[1] + '\n\nsaved:' + el[2] + '\n\nnow : ' + dataNow,'check Saved File failture(element:)') == false) break;
    }
  }
...
scrapeHeyzo = (res) => {
  ...
  if (video) {
    ...
    if (!$('#append').val().match(sample))
      setTextareaValue('append',sample + $('#append').val());
  }
  ...
  if ($('#title').val() == '') $('#title').val((item[2] + '　' + item[5]).normalize( "NFC" ));
...
var readOkashiLemon = (res,osnap,pid,urlData) => {
  ...
  for (var i in arr) {//arr.length;i++){
    ...
    if ($('#title').val().length == 0) {
      $('#title').val(ar[2].replace('/','　').normalize( "NFC" ));
    }
    if (ar[7].length > 0) {
...
$('.restartAutoSave').on('click', () => {
	autoSaveNew(true);
	aModal('設定を変更しました', 'restart AutoSave');
	autoSaveNew();
})
...
var readSavedFile = async (fileName) => {// "on load" and click button 
	if (!fileName || (fileName.length < 1)) {
		var fileName = $('#title').val().trim().substring(0, 32).replace(/[:\s|\/]/g, '_') + '.txt';
		if (fileName == '.txt') fileName = 'untitled.txt';
	}

	var savedFileData = await promisePHP(baseDataFolder + fileName, 'thumbnail');
	var tempFileData = await promisePHP(baseDataFolder + tempFolder + tempPrefix + fileName,'thumbnail');

	if (tempFileData.indexOf('は存在しません') > 0) {
		if (savedFileData.indexOf('は存在しません') > 0) return;// no file
			$('#savedOrTemp').html('Saved File のみ');
			await fetchTempFile(baseDataFolder + fileName, 'modal');// only saved file
	} else {
		//console.log('tempFileData',tempFileData)
		if (savedFileData.indexOf('は存在しません') > 0) {// only temp file
			$('#savedOrTemp').html('Temp File のみ');
			await fetchTempFile(baseDataFolder + tempFolder + tempPrefix + fileName, 'modal');
		} else { //both exist
			var tempFileTime = new Date(tempFileData.match(/(\d{4}\-\d{2}\-\d{2}\s?\d{2}:\d{2}:\d{2})/)[1]);//.replace(/-/g, '/');
			console.log('tempFileTime', tempFileTime)
			var savedFileTime = new Date(savedFileData.match(/(\d{4}\-\d{2}\-\d{2}\s?\d{2}:\d{2}:\d{2})/)[1]);//.replace(/-/g, '/');
			console.log('savedFileTime', savedFileTime);
			console.log('file time compare Temp - Saved => ',tempFileTime - savedFileTime)
			if (tempFileTime > savedFileTime) {
				$('#savedOrTemp').html('Saved File あり');
				await fetchTempFile(baseDataFolder + tempFolder + tempPrefix + fileName, 'modal');
			} else {
				$('#savedOrTemp').html('Temp File あり');
				await fetchTempFile(baseDataFolder + fileName, 'modal');
			}
		}
	}
}
...
var modalSaved = (rText,page = 0) => {
  if (rText.match('failed to open stream')) {
	...
	$('#txtSaved').val(rText);
	if (!showSavedData(page))
		return false;
  return true;
}

$('#buttonSaveBefore').on('click', () => {
	var num = parseInt($('#saveHistoryPage').val());
	console.log('buttonSaveBefore clicked num => ', num)
	if ((num == 0) || (isNaN(num))) return;
	showSavedData(num - 1);
});

$('#buttonSaveAfter').on('click', () => {
	var num = parseInt($('#saveHistoryPage').val());
	console.log('buttonSaveAfter clicked num => ', num)
	showSavedData(num + 1);
});

var showSavedData = async (num) => {
	var rText = $('#txtSaved').val();
	var filename = $('#savedFileName').val();
	console.log('showSavedData read daved file',filename)
	//var rText = await promisePHP(filename, 'read');
	if (!rText) alert("showSavedData error read file");
	...
	var nextBlock = txtBlock[num + 1];
	var nextDate;
	if (nextBlock) nextDate = nextBlock.match(/<!-- data start (\d{4}_\d{1,2}_\d{1,2} \d{1,2}:\d{1,2}:\d{1,2})/);
	if (!nextBlock || !nextDate) $('#buttonSaveAfter').attr('disabled', true);
	else $('#buttonSaveAfter').removeAttr('disabled');
	if (num == 0) $('#buttonSaveBefore').attr('disabled', true);
	else $('#buttonSaveBefore').removeAttr('disabled');

	var txt = txtBlock[num].split('\t');
  var same = true;
	var startDate = txtBlock[num].match(/<!-- data start (\d{4}_\d{1,2}_\d{1,2} \d{1,2}:\d{1,2}:\d{1,2})/);
	if (startDate) {
		$('#savedDate').val(startDate[1]);
	} else {
		//console.log('データがありません num => ' + num + '\ntxtBlock => ' + txtBlock[num],'textSaved => ',$('#txtSaved').val())
		await myAlert('データがありません num => ' + num + '\ntxtBlock => ' + txtBlock[num],'データがありません ' + filename,'閉じる');
		return false;
	}
	
	$('#saveHistoryPage').val(num.toString());

	$('#textSaved').empty();
  for (var line of txt) {
		var el = line.match(/<!-- (.+?) -->([\S\s]*)/);
		if (!el || (el[1] == 'ofile') || el[1] == 'category') continue;
		//console.log('line of readfile 5824',line,'el0==>',el[0],'el1===>',el[1],'element value =====>',$('#' + el[1].trim()).html(),'el2====>',el[2])
		if (el[1] == 'ofile') continue;
		var saved = el[2];
		if (!el[2]) saved = '';
    saved = saved.replace(/\r\n/g,'\n').trim();
		var now = $('#' + el[1]).val();
		if ($('#' + el[1]).hasClass('wEditor'))
			now = $('#' + el[1]).trumbowyg('html');
		if (!now) now = '';
		now = now.replace(/\r\n/g,'\n').trim()
		/*if ($('#' + el[1]).val()) {*/
      /*if (now) {*/
        ...
        if (now.length > saved.length) {
          ...
          savedColor = 'color: #faa;';
				...
				//var randomId1 + 'saved' = 'n' + Math.random().toString(36).slice(-8);

        $('#textSaved')
        .append($('<div>',{
          class: 'text-warning font-italic font-bold',
          text: holder}))
        .append($('<div>',{class: "form-inline mb-3 now-data"})
            ...
            .append($('<div>',{class: "row"})
              ...
              .append($('<span>',{
                text: ' : ' + now.length + '文字',
							...
							.append('<i class="fa-solid fa-person-walking-arrow-right btn text-warning mx-1 my-0 py-0" onclick="copyNowToSaved(\'' + el[1] + '\')"></i>'))
							.append($('<div>',{class: "trumbowyg-dark w-100"})
            .append($('<textarea>',{
							class: 'form-control dark m-0 p-0 trumbowyg-dark wEditor',
							//name: el[1],
							id: 'nowData-' + el[1],
              placeholder: el[1],
          ...
          .append($('<div>',{class: "col-sm-offset-0 col-sm-6"})
            .append($('<div>',{class: "row saved-data"})
              .append($('<label>',{for: 'comt',text: '保存テキスト'})
              .append($('<span>',{
                text: ' : ' + saved.length + '文字',
								style: savedColor
							}))
								.append('<i class="fa-solid fa-person-circle-minus btn text-danger mx-1 my-0 py-0" onclick="deleteSavedData(' + el[1] + ')"></i>'))
							.append($('<div>',{class: "trumbowyg-dark w-100"})
								.append($('<textarea>',{
									class: 'form-control dark m-0 p-0 wEditor w-100',
									id: 'savedData-' + el[1],
									placeholder: el[1],
									style: 'resize: vertical; width:100%;',
									text: saved,
		...
		console.log(el[1].type,el[1])

		$('#' + 'nowData-' + el[1]).trumbowyg({
			lang: 'ja',btns: [['viewHTML'],['fullscreen']]
		});

		$('#' + 'savedData-' + el[1]).trumbowyg({
			lang: 'ja',btns: [['viewHTML'],['fullscreen']]
		});
				
		$('#' + 'nowData-' + el[1]).toggleClass('trumbowyg-dark');
		$('#' + 'savedData-' + el[1]).toggleClass('trumbowyg-dark');	
		$('#' + 'nowData-' + el[1]).trumbowyg('toggle');
		$('#' + 'nowData-' + el[1]).val(now);
		$('#' + 'savedData-' + el[1]).trumbowyg('toggle');
		$('#' + 'savedData-' + el[1]).val(saved);

			//alert('>' + '#' + el[1] + '<>' + el[1].trim() + '<>' )
			//await myAlert(el[1],$('#' + el[1]).val())
	}
  ...
  if (same) {
    aModal('最終保存と同じ','modalSaved');
		$('#same').text('現在のページと同じ');
		$('#same').css('color', '#4a4');
		$('#loadFileButton').css('display', 'none');//attr('disabled', true);
    return 'same';
  }
  $('#same').text('現在のページと異なります');
	$('#same').css('color', '#a44');
	$('#loadFileButton').css('display', 'block'); //removeAttr('disabled');
	return 'different';
}
...
var deleteSavedData = (ele) => {
	console.log(ele.type, ele)
	if (ele.type == 'textarea') ele = $(ele).attr('id');
	console.log(ele.type,ele)
	$('#savedData-' + ele).val('');
}

var copyNowToSaved = (ele) => {
	console.log(ele.type, ele)
	if (ele.type == 'textarea') ele = $(ele).attr('id');
	console.log(ele.type,ele)
	$('#savedData-' + ele).val(
...
var loadFile = (page = 0) => {
	...
	//if ($('#same').text() == '現在のページと同じ') {
	//	return;
	//}
	var rText = $('#txtSaved').val();
	if (!rText) alert("loadfile error read file");
  ...
  for (var line of txt) {
    var el = line.match(/<!-- (.+?) -->([\S\s]*)/);
		if (el) {
			...
			var savedData = $('#savedData-' + el[1]).val();
			//console.log("5987 ===> ",savedData)
			if (savedData) setTextareaValue(el[1],savedData)
			else setTextareaValue(el[1], el[2]);
			//$('#' + el[1]).val(el[2]);
			//$('#' + el[1]).trumbowyg('html', el[2]);
    }
...
easeSurugaya = async (urlData) => {
  ...
  var img = $('#img').val();
  var url = '<a href="https://affiliate.suruga-ya.jp/modules/af/af_jump.php?user_id=1861&amp;goods_url=https%3A%2F%2Fwww.suruga-ya.jp%2Fproduct%2Fdetail%2F' + spd[1] + '" rel="nofollow" target="_blank" rel="noopener noreferrer">';
  ...
  if (item) {
		if ($('#title').val() == '') $('#title').val(item[1].replace(' / ', '　')
			.replace(/[ 　]?（ブルーレイディスク）|[　 ]?【数量限定】|[　 ]?【ベストヒッツ】|[ 　]?【特選アウトレット】|[ 　]?Blu-ray版|[　]?【アウトレット】|[ 　]?（DOD）/g, '').normalize( "NFC" ));
  ...
  var url = '"url": "(.+?)"';
  if (url) {
    var pno = url[1].substring(url[1].lastIndexOf('/'));
...
var nyoshin = async (urlData) => {
		...
		if (pId) {
			...
			if (append.indexOf(imgUrl) < 0) setTextareaValue('append',append + a);
			var res = urlData.indexOf("/movie");
			var url = '<a href="https://click.dtiserv2.com/Direct/9361999-361-184037' + urlData.substring(res) + '" target="_blank" rel="noopener noreferrer"><img src="' + imgUrl + '" style="max-width: 500px;">「女体のしんぴ」で詳細を見る<br><br></a>';
			var img = $('#img').val();
  ...
  if (title) if ($('#title').val() == '') $('#title').val(title[1].normalize( "NFC" ));
...
var fc2PHP = async (urlData) => {
	...
	setTextareaValue('append','<iframe width="100%" height="394" style="max-width:100%;" src="https://adult.contents.fc2.com/embed/' + sp[1] + '?i=TVRjM05UUTFORFE9&info=0&logo=0" frameborder="0" allowfullscreen></iframe>','append');
	var pno = $('#p-no').val();
	if (pno.length > 2) pno += '、';
	if (pno.indexOf(sp[1]) < 0)
		$('#p-no').val(pno + 'FC2 PPV ' + sp[1]);
	var img = '<div class="sample_block"><a href="https://adult.contents.fc2.com/aff.php?aid=' + sp[1] + '&affuid=TVRjM05UUTFORFE9" target="_blank" rel="noopener noreferrer">' + $('#title').val() + '<br><br>FC2で詳細を見る</a></div><br>';
	...
	if (title && ($('#title').val().length < 5)) {
		...
		if (fc) title[1] = fc[1];
		$('#title').val(title[1].normalize( "NFC" ));
	}
	...
	var imgURL = '';
	var images = txt.match(/https:\/\/storage.+?\/\S+?\.[jpen]{2,3}g/g);
	if (images) {
		for (var image of images) {
			console.log('FC2 imgeURL',image, imgURL.indexOf(image))
...
var scrapeThis = async (ele,long = null) => {
	if (ele == 'p-no') {
		...
		while (pno.match(/^[\/\-\.\)\s_]/)) { //start
			pno = pno.slice(1);
		}
		while (pno.match(/[\/\-\.\(\s_]$/)) { // end
			pno = pno.slice(0,-1);
		}
		$('#p-no').val(pno);
		$('#orgNo').val(pno);

		if (((pno != '') && pno.toUpperCase().indexOf('1PONDO') < 0) &&
			(pno.toUpperCase().indexOf('CARIB') < 0)  && (pno.toUpperCase().indexOf('NYOSHIN') < 0) &&
			pno.match(/[a-z]/) && await myConfirm(pno,'大文字にしますか？','はい','しません'))
  ...
  $(event.target).val('');
	...
	setTimeout(function () {
		...
		switch (event.target.id) {
			...
			case 'imgURL':
				if (!ap) {
					var img = txt.match(/(http[^<]+?jpe?g)/);
					if (img) {
						txt = '<img src="' + img[1] + '">';
						console.log('imggg',txt)
						$('#imgURL').val($('#imgURL').val() + txt);
					}
				} else {
					img = txt.match(/(http[^<]+?png)/);
					if (img) {
						txt = '<img src="' + img[1] + '">';
						console.log('imggg',txt)
						$('#imgURL').val($('#imgURL').val() + txt);
			...
			case 'dropImageInput':
				...
				break;
			default:
				if (txt.indexOf('.jpg') > 0) {
					setTextareaValue('append',$('#append').val() + snapImage(txt));
				}
		}
	}, 300);
	//alert(txt);
...
var addImage = () => {
  ...
  console.log('#addImage click',img);
	displayImage(img);
	setTextareaValue('append',$('#append').val() + snapImage(img));
	$('#imgDrop').val('');
}
...
var displayUrl = () => {
  ...
  var title = url.substring(0,10);
  var name = url.match(/(.+)\((.+?)\)$/);
  if (name) {
    title = name[1].substring(0,10);
    url = name[2];
  }
...
async function scrapeCarib(urlData) {
  ...
  if (title && ($('#title').val().length == 0)) {
		$('#title').val(title[1].normalize("NFC")
			.replace(' - 無修正動画 カリビアンコム ', '')
			.replace(' | 無修正アダルト動画 カリビアンコム', '').trim());
  }
...
//  }
  ...
  if (pID) {
    ...
    //<a href="http://click.dtiserv2.com/Direct/8881889-6-184037/moviepages/072315-927/index.html" target="_blank" rel="noopener noreferrer"><img src="https://www.caribbeancom.com/moviepages/072315-927/images/l_t.jpg" border="0" hight="394" width="100%"></a>
    var img = '<a href="http://click.dtiserv2.com/Direct/8881889-6-184037/moviepages/' + pID[1] + '/index.html" target="_blank" rel="noopener noreferrer"><img src="https://www.caribbeancom.com/moviepages/' + pID[1] + '/images/l_t.jpg" border="0" height="394" width="100%">CaribbeanComで詳細を見る<br><br></a>';
    if (!$('#img').val().match(img)) $('#img').val($('#img').val() + img);
    //</moviepages/121818-812/images/l/002.jpg
    var video = '<video width="100%" controls poster="https://www.caribbeancom.com/moviepages/' 
    + pID[1] + '/images/l_t.jpg" src="http://smovie.caribbeancom.com/sample/movies/' 
    + pID[1] + '/480p.mp4"><p>ご使用のブラウザでは動画再生に対応していません</p></video>';
    if (!$('#append').val().match(video)) setTextareaValue('append',video + $('#append').val());
    ...
    if (images) {
      for (var image of images) {
        await displayImage('https://www.caribbeancom.com' + image.substring(1));
        console.log(image);
				if (append.indexOf(image) < 0)
					append += snapImage('https://www.caribbeancom.com' + image.substring(1));
			};
			append += '<br style="clear: both;">\n\n';
    }
		$('#imgURL').val($('#imgURL').val() +  snapImage('https://www.caribbeancom.com/moviepages/' + pID[1] + '/images/l_l.jpg'));
...
async function scrapeCaribComp(urlData) {
  ...
  if (pID) {
    ...
    var img = '<a href="http://click.dtiserv2.com/Direct/8881888-290-184037/moviepages/' 
      + pID[1] + '/index.html" target="_blank" rel="noopener noreferrer"><img src="https://www.caribbeancompr.com/moviepages/' + pID[1] + '/images/l_l.jpg" border="0" height="394" width="100%">Caribcompで詳細を見る<br><br></a>';
    if (!$('#img').val().match(img)) $('#img').val($('#img').val() + img);
...
var promisePHP = (filename, mode = 'scrape', dir = '', count = 20) => {
	console.log('fileneme mode dir count',filename,mode,dir.length,count)
  return new Promise((resolve,reject) => {
...
var scrapeDuga = async (urlData) => {
	...
	if (images) {
		console.log(images)
		for (var image of images) {
			append += snapImage(image.substring(9));
		}
//		append += '<br style="clear: both;">';
		setTextareaValue('append',append);
	}
...
var scrapeIchigo = async (urlData) => {
  //------------ Ichigo Daifuku -----------
	...
	var afi = '<a href="https://wlink.golden-gateway.com/stlink/9901-1-002-87af' + url[1] + '" target="_blank" rel="noopener noreferrer">いちごキャンディーで詳細を見る<br><br></a>';
	$('#img').val($('#img').val() + afi);
...
$('.orgButton').on('click', function (event) {
  ...
  if (id) {
    ...
    if (url.length > 5) {
			$('#exampleFrame').append('<iframe name="exampleWindow" style="width:100%" allow="fullscreen *"></iframe >');
			window.open(url, 'exampleWindow', 'menubar=yes,toolbar=yes,location=yes,status=yes,resizable=yes,scrollbars=yes');
    }
  }
})
  ...
  if (tab) {
    ...
    if (tab[2] == 'Url') {
      ...
      switch (tab[1]) { // with sitename
        ...
        case 'Girl':
				case 'MyGirl':
          url = 'https://nomado.moo.jp/girls/?s=' + show;
          break;
				case 'Lemon':
      ...
      if (show == '') return;
		...
		wObj = window.open(url, '_blank', 'menubar=yes,toolbar=yes,location=yes,status=yes,resizable=yes,scrollbars=yes,width=' + wWidth + ',height=800,');
...
$('#gTranslate').on('click', () => { //  translate
	//setTextareaValue('Japanese',$('#feel').trumbowyg('html'));
  $('#Japanese').val($('#feel').val());
	//setTextareaValue('translated', $('#feel').val());
  ...
  //console.log('feel = ', $('#feel').val())
	...
	$('#btn-inner').val('メインに転記する');
	console.log('メインに転記する')
	tenki();

	startTestTimer();
});

$('#Japanese').on('tbwchange', () => {
	console.log('Japanese on click 7141')
	tenki();
});
...
$('#Japanese').on('keyup', function () {
	...
	tenki();
	//setTextareaValue('translated',$('#Japanese').trumbowyg('html').replace(/\n/g, '<br>'));
...
var tenki = () => {
	//setTextareaValue('translatedText',
  $('#translatedText').val(
    $('#translated').html()
...
$('#translateOK').on('click', function () {
	doGTranslate('ja|ja');
	setTextareaValue('japaneseOld',$('#Japanese').val());
	//$('#japaneseOld').val($('#Japanese').val());
	setTextareaValue('english',$('#translatedText').val());
	//$('#english').val($('#translatedText').val());
  //$('#english').trumbowyg('html',$('#translatedText').val());

	$('#translateModal').modal('hide');
	stopTestTimer();
	$('#changed').html('*english');
	waitIcon();
	$('#oldDateTime').val(Date.now().toString());
  return true;
})
...
const myConfirm = (() => {
	...
	return (text,title = '',yes = 'はい',no = 'いいえ') => {
		p.textContent = text;
...
var myConfirmF = async (mes = '続けていいですか', title, yes = 'OK', no = 'いいえ', mass) => {
	new Promise((resolve,reject) => {
		resolve();
	}).then(() => {
		console.log('resolve したよ')
	})
	if (mass) {
...
const myPrompt = (() => {
	...
	return (text, intialValue = "",yes = 'OK',no = 'キャンセル') => {
		p.textContent = text;
		alt.style.display = "block";
...
var displayEnglish = () => {
	...
	if (!display || (display == "none")) ele.style.display = "block"; 
	else ele.style.display = "none";
	console.log("display english", display, ele.textContent.length); 
}

var displayAppend = (no) => {
	var eles = ['engText', 'appendText'];
	//var icons = ['engText']
	var ele = document.getElementById(eles[no]);
	var icon = document.getElementById(eles[no] + 'Icon');
	var display = ele.style.display;
...
var replaceParagraph = (ele) => {
	...
	setTextareaValue(ele, txt);
	//$('#' + ele).val(txt);
	//$('#' + ele).trumbowyg('html', txt);
}
...
var addClear = (ele = 'img') => {
	...
	if (img.indexOf(clear) < 0)
		setTextareaValue(ele, img + clear);
		//$('#' + ele).val(img + clear);
...
var formatProf = () => {
	...
	for (var i = 0; i < prof.length; i++) {
		var item = prof[i].trim().match(/([\s\t]*)(\S+?)[\s\t]*[:： 　](.+)/);
		...
		if ((item) && (item.length == 4) && (prof[i].indexOf('生年月日') < 0)) {
			switch (item[2].trim()) {
				...
				case '公開日':
				case '販売日':
					if ($('#date').val() == '') $('#date').val(item[3].trim());
					break;
				case 'タグ':
				case 'ジャンル':
				case 'カテゴリー':
					var tags = ['オリジナル動画', '寝取り・寝取られ・NTR', 'NTR', 'AV女優', '鬼畜', '近親相姦', 'ハーレム', 'オムニバス', 'ギャル', '淫語', '微乳', '美脚', 'モデル系', '幼なじみ', 'メイド', '看護婦・ナース', '熟女・人妻', '人妻・主婦', '若妻・幼妻', '主婦', '若妻', '幼妻', '熟女', '人妻', '日焼け', 'パイパン', '痴女', '口内発射', '水着','ロリ系', 'ロリ', '美尻', 'スレンダー', '巨乳', '放尿・お漏らし', 'お漏らし', '放尿', '美乳', 'ハード系', 'レズ', 'パイズリ', 'ごっくん', 'アナル', '手コキ', 'マングリ返し', 'コスプレ', 'ぶっかけ', '69', '乱交', '顔射', 'オナニー', '潮吹き', '初裏', '指マン', 'パンチラ', '姉・妹', 'お姉さん', '生ハメ・生姦', '中出し', 'バイブレーター', 'バイブ', 'クンニ', 'ブルマー', '制服', 'セーラー服', 'ランジェリー', 'イラマチオ', 'フェラチオ', 'フェラ', '3P・4P', '3P', '３P', 'ストリーミング対応', '1080p', 'fps', 'フェチ', 'カップル', '主観', '辱め', '女子校生', 'めがね', 'メガネ', '学生服', '和服・浴衣', '野外・露出', '露出', '温泉', '羞恥', '美少女', '女子大生', 'マッサージ・リフレ', 'ローション・オイル', '貝合わせ', '騎乗位', '顔面騎乗', '逆3P', '口内射精', 'その他', '性転換・女体化', '局部アップ', '職業色々', '企画', '局部アップ', 'イタズラ', 'いたずら', 'デジモ', '巨尻', 'レオタード', '妄想', 'アイドル・芸能人', 'アイドル', '芸能人', 'イメージビデオ', '単体作品', '作品', '動画', '飲み会・合コン', '飲み会', '合コン', 'おもちゃ', 'お母さん', 'ショタ', 'ドキュメンタリー', '上司', 'ハメ撮り'];
					...
					item[3] = item[3].trim().replace(/\s/g, '')
						.replace(/、・/g, '・').replace(/、{2,}/g,'、');
					item[3] = item[3].replace('フェラ、チオ', 'フェラチオ');
					if (item[3].slice(-1) == '、') item[3] = item[3].slice(0, -1);
					break;
				case '出演':
				case '出演者':
				case '出演女優':
...
var divideCopy = async (ele) => {
	...
	var txt = txts[0];// + '>';
	var txtsLength = txts.length;
	for (var i = 1; i < txtsLength; i++) {
		...
		if (txt.length + txts[i].length < num) {
			...
			$('#divideCopy').append(
				'<div class="row form-group dividedText1"><button type="button" class="btn btn-outline-warning col-sm-1 m-0 p-0 divideButton" onclick="divideCopyButton(event)">コピー' + i + '<i class="fa-solid fa-clone"></i></button><textarea class="col-sm-11 dark">' + txt + '</textarea></div>'
			);
			if (txts[i] != '')
				txt = '<' + txts[i];
		}
	...
	$('#divideCopy').append(
		'<div class="row form-group dividedText1"><button type="button" class="btn btn-outline-warning col-sm-1 m-0 p-0 divideButton" onclick="divideCopyButton(event)">コピー' + txtsLength + '<i class="fa-solid fa-clone"></i></button><textarea class="col-sm-11 dark">' + txt + '</textarea></div>'
	);
...
var minnaProfile = (res, long) => {
	...
	var items = ['愛称','別名','生年月日', 'サイズ', '血液型', '出身地', '所属事務所', '趣味・特技','趣味／特技', 'AV出演期間', 'デビュー作品', 'ブログ', '公式サイト'];
		...
		for (var multi of multis) {
			...
			if (item) {
				if (name == '生年月日') {
					...
					var furigana = item[1].match(/(.+?)([\s　]*?[\(（].+)/);
					if (furigana) {
						if (ret.indexOf(furigana[1]) < 0) ret += '[' + furigana[1] + '](G)' + furigana[2] + '、';
					}
					else if (ret.indexOf(item[1]) < 0) ret += '[' + item[1] + '](G)、';
...
$('.trumbowyg-button-group > .trumbowyg-viewHTML-button').on('click', function (event) {
	alert(event.target)
	console.log('wew',event)
	console.log($(event.target).next().val());
	let url = $(event.target).next().val();
...
//Editorが変更される時に、いったん避難して、書き戻す関数。
...
var editOnSideBar = () => {
	//$('.appendPHP').click();
	//saveOnClick();
	var eles = ['title', 'date', 'p-np', 'p-id', 'img', 'feel', 'english', 'prof', 'cmt', 'append', 'url', 'imgURL', 'ofile', 'n'];
	for (var ele of eles) {
		$('#tree-' + ele).val($('#' + ele).val());;
		$('#tree-' + ele).trumbowyg('html',$('#' + ele).val());
	}
/*	$('#tree-')
	$('#tree-title').val($('#title').val());
	$('#tree-date').val($('#date').val());
	$('#tree-p-no').val($('#p-no').val());
	$('#tree-p-id').val($('#p-id').val());
	$('#tree-img').val($('#img').val());
	$('#tree-feel').val($('#feel').val());
	$('#tree-english').val($('#english').val());
	$('#tree-prof').val($('#prof').val());
	$('#tree-cmt').val($('#cmt').val());
	$('#tree-append').val($('#append').val());
	$('#tree-url').val($('#url').val());
	$('#tree-imgURL').val($('#imgURL').val());
	$('#tree-ofile').val($('#ofile').val());
	$('#tree-n').val($('#n').val());
  */
	//$('.nav-pills a[href="#tabE"]').tab('show');
//var someTabTriggerEl = document.querySelector('.nav-pills a[href="#tabE"]');
...
var treeClear = () => {
	var eles = ['title', 'date', 'p-np', 'p-id', 'img', 'feel', 'english', 'prof', 'cmt', 'append', 'url', 'imgURL', 'ofile', 'n'];
	for (var ele of eles) {
		$('#tree-' + ele).trumbowyg('empty');
		$('#tree-' + ele).val('');
	}
}

$('#loadFromFile').on('click', async () => {
	var filename = $('#ofile').val();
	var slash = filename.match(/\\([^\\]+?\.txt)/);
	if (slash) {
		console.log('slash => ', slash);
		var pass = baseDataFolder;
		if (filename.indexOf(tempPrefix) >= 0) pass += tempFolder;
		console.log('filename =>',pass + slash[1],filename)
		var rText = await promisePHP(pass + slash[1], 'read');
		if (!modalSaved(rText))
			await myAlert(pass + slash[1] + '\n\nmodal表示エラー','ファイルが開けません');
	} else {
		await myAlert(filename + '\n\nfileがありません','ファイルが開けません');
	}
})

var setTextareaValue = (ele, txt, append = 'overwrite') => {
	if (append == 'append') {
		var over = $('#' + ele).val();
		if (over != '') {
			txt = over + '\n\n' + txt;
...
$('#readTempFile').on('click', async () => {
	var fileName = $('#savedFileName').val();
	if (fileName.indexOf(tempPrefix) >= 0) {
		if (await myConfirm('再読込しますか') == false) return;
		//fileName = fileName.substring(4);
	} else fileName = fileName.replace(baseDataFolder, baseDataFolder + tempFolder + tempPrefix);
	//fileName = 'data/tmp_data/tmp_' + fileName;
	if(await promisePHP(fileName,'isfile') )
	var rText = await promisePHP(fileName, 'read');
	$('#savedFileName').val(fileName);
	modalSaved(rText);
		//await myAlert('data/tmp_data/tmp_' + fileName + '\n\nmodal表示エラー','ファイルが開けません');
})

$('#readSavedFileModal').on('click', async () => {
	var fileName = $('#savedFileName').val();
	if (fileName.indexOf(tempPrefix) < 0) {
		if (await myConfirm('再読込しますか') == false) return;
	} else fileName = fileName.replace(baseDataFolder + tempFolder + tempPrefix,baseDataFolder);
	//fileName = baseDataFolder + fileName;
	var rText = await promisePHP(fileName, 'read');
	$('#savedFileName').val(fileName);
	modalSaved(rText)
		//await myAlert(baseDataFolder + fileName + '\n\nmodal表示エラー','ファイルが開けません');
})

var getFileInfo = (fileName, fileType = '') => {
	//fileName of base,tmp_, fileType '',tempfolder + prefix,completefolder,
	var data = {filename: fileName, filesize: 'filesize', filetime: 'filetime', thumb: 'thumb'}
	return new Promise(async (resolve, reject) => {
		var lastSlash = fileName.lastIndexOf(fileName, '/');
		if (lastSlash >= 0) fileName = fileName.substring(lastSlash + 1);
		var res = await promisePHP(baseDataFolder + fileType + fileName, 'thumbnail');
		if (!res) reject(false);
		if (res.indexOf('が存在します') > 0) {
			data.filename = fileName;
			var fileSize = res.match(/filesize=(\d+?)byte/);
			if (fileSize) data.filesize = fileSize[1];
			var fileTime = res.match(/(\d{4}\-\d{2}\-\d{2} \d{2}:\d{2}:\d{2})<time>\n(.+)/);
			if (fileTime) {
				data.filetime = fileTime[1];
				data.thumb = fileTime[2];
			}
		} else data.filename = false;
		resolve(data);
	})
}

var displayPageAlt = async () => {//別ページで表示
	var exampleHtml = 'exampleMake.html'
	var basicHtml = await promisePHP('basic.html', 'read');
	if ((!basicHtml) || (basicHtml.indexOf('読み込みに失敗しました') >= 0)) {
		await myAlert(basicHtml, '読み込みに失敗しました basic.html');
		return (false);
	}

	basicHtml = basicHtml.replace('Example of codeMaker', $('#title').val());
	//console.log('read basic.html', basicHtml);
	//await myConfirm(basicHtml, 'read basic.html success');
	var start = basicHtml.indexOf('<!DOCTYPE html>');
	if (start > 0) basicHtml = basicHtml.substring(start);
	//basicHtml += '<p class="h1 title text-center">' + $('#title').val() + 'タイトルじゃけん</p>\n';
	//console.log('befor make 7765');
	var res = await makeCode(false, 'moon');
	basicHtml += '<div class="h3">' + $('#title').val() + '</div>'
	basicHtml += res;
	//console.log('after make 7767',res)
	basicHtml += '</div>\n</body>\n</html>';

	var res = await promisePHP(exampleHtml, 'overwrite',basicHtml);
	if (res.indexOf('write file error. overwrite') >= 0) {
		await myAlert(res, 'write file error. overwrite. ' + exampleHtml);
		return;
	}
	window.open('./' + exampleHtml, '_blank', 'menubar=yes,toolbar=yes,location=yes,status=yes,resizable=yes,scrollbars=yes');
}

var syncedIcon = () => {
	$('#statusIcon').html('<i class="fa-solid fa-sync fa-fade text-info" style="--fa-animation-duration: 2s;"></i>');
}

var savingIcon = () => {
	$('#statusIcon').html('<i class="fa-solid fa-pencil fa-shake text-danger"></i>');
}

var waitIcon = () => {
	$('#statusIcon').html('<i class="fa-regular fa-clock fa-beat text-warning"></i>');
}

$('.itemStatus .dropdown-item').click(function() {
	console.log(this)
	var item = $(this).html();
	console.log(item)
...
var shousaiMiru = (web) => {
	console.log('8064 before exec', web)
	var sites = web.match(/<a href="(http.+?)"[\s\S]+?>([^>]+?で詳細を見る)/gu);
	if (!sites) {
		//await myAlert('サイトが見つかりません');
		return(web);
	}

	for (var i = 0; i < sites.length; i++) {
		console.log('8072 after exec sites ', sites[i])
		var htmlSite = sites[i].match(/<a href="(http.+?)"[\s\S]+?>([^>]+?で詳細を見る)/);
		console.log('8074 html ', htmlSite)
	}
	var imageUrl = web.match(/<img src=".+?>/);
	console.log('8077 image ', imageUrl)
	if (!imageUrl) {
		//await myAlert('イメージがありません');
		return(web);
	} else {
		if (imageUrl[0].indexOf('align="left"') < 0)
			imageUrl[0] = imageUrl[0].replace('>', ' align="left">');
		console.log('8083 image ', imageUrl)
	}

	var itemName = web.match(/<!-- Amazon title -->(.*?)<!-- Amazon title end -->/); //alt="(.+?)"
	console.log('8086 itemName', itemName)
	if (!itemName) {
		itemName = web.match(/alt="(.*?)"/);
	}

	console.log('8090 itemName', itemName)
	var htmlSite = sites[0].match(/(<a href="http.+?)"[\s\S]+?>([^>]+?で詳細を見る)/);
	if (!htmlSite) {
		aModal('サイトがありません', 'image');
		return(web);
	}
	var webDetail = htmlSite[1] + '" target="_blank">' + imageUrl[0] + '</div>';
	if (itemName) {
		webDetail += itemName[1] + '</a><br><br>'
		var itemBrand = web.match(/<div style="color: black;"><!-- Amazon publisher -->.+?<!-- Amazon publisher end -->/);
		if (itemBrand) webDetail += itemBrand[0] + '<br><br>';
	}
	webDetail += '<span style="font-size: +1;">詳細を見る</span><br>'
	for (var i = 0; i < sites.length; i++) {
		console.log('8106 after exec sites ', sites[i])
		var htmlSite = sites[i].match(/(<a href="http.+?>).*?([^>]+?)で詳細を見る/);//<a href="(http.+?)"[\s\S]+?>([^>]+?で詳細を見る)
		if (htmlSite) {
			webDetail += '<span class="linkTable">' + htmlSite[1] + htmlSite[2] + '</a></span>';
		}
		console.log('8111 html ', htmlSite)
	}
	webDetail += '<br><br>';
	console.log('8113 webDetail ', webDetail)
	return(webDetail);
} 

var dlsite = (urlData,flag = true) => {
	var url = urlData.match(/id\/(.+?).html/);
		console.log(urlData,flag)
	if (url) {
		var affi = '<a href="https://www.dlsite.com/maniax/dlaf/=/t/i/link/work/aid/charbo/id/' + url[1] + '.html" target="_blank" rel="noopener sponsored"><img src="//img.dlsite.jp/modpub/images2/work/doujin/' + url[1] + '_img_main.jpg" itemprop="image" alt="dlsite" border="0" class="target_type" />DLsiteで詳細を見る<br><br></a>';
		$('#img').val(affi);
		$('#imgURL').val($('#imgURL').val() + '<img itemprop="image" src="//img.dlsite.jp/modpub/images2/work/doujin/' + url[1] + '_img_main.jpg" alt="" border="0" class="target_type" />');
		console.log(url,flag)
	} else {
		aModal('DLsite url が見つかりません', 'easeUp');
	}
}


var autoSaveNew = async (stop = false) => {
	var second = parseInt($('#timeSpan').val());
	var char = parseInt($('#charSpan').val());
	...
	if (!isNaN(timerNo) && stop) {
		clearInterval(timerNo);
		$('#autoSaveNumber').val('');
		console.log('autoSave Stopped', timerNo);
		return (true);
	}


	var autoSaveFunction = async () => {//　char文字数&minute秒後
		var textLengthNow = allToText().length;
		var timeNow = Date.now();
		var changeTime = $('#oldDateTime').val();
		var changeEle = $('#changed').html();
		var second = parseInt($('#timeSpan').val());

		/*if (((textLengthNow - textLengthOld > char) || (textLengthOld - textLengthNow > char)) && (changeEle.indexOf('*') >= 0)) { // 文字数超過
			console.log('autosave count = ' + count, textLengthNow, textLengthOld, char);
			var awaitSave = await saveOnClick('append', $('#changed').html());
			textLengthOld = textLengthNow;
			$('#oldDateTime').val(Date.now().toString());
			return (awaitSave);
		}
		...
		if ((changeEle.indexOf('*') >= 0) && ((timeNow - changeTime) / 1000 > second)) {// 最後の変更 second秒後
			console.log('8221 autoSave New after imeSpan ', (timeNow - changeTime) / 1000, second);
			if (textLengthNow != textLengthOld) { // textがちがう時
				//$('#changed').html('NS').css('color', '#403');
				console.log('texts differ time 8232 ', changeEle,textLengthOld,textLengthNow, (textLengthNow != textLengthOld))
				
				var awaitSave = await saveOnClick('overwrite', changeEle);
				console.log('texts differ time 8235 after ', changeEle,textLengthOld,textLengthNow, (textLengthNow != textLengthOld),awaitSave)
				syncedIcon();
				$('#oldDateTime').val(Date.now().toString());
				textLengthOld = textLengthNow;
				return (awaitSave);
				...
				//if ((textLengthOld = textLengthNow) && (changeEle.indexOf('*') >= 0)) {
				$('#changed').html('');
				syncedIcon();
				$('#oldDateTime').val(Date.now().toString());
				//}
			...
			//console.log('8221 autoSave New after imeSpan ', (timeNow - changeTime) / 1000, second,' LENGTH ',textLengthOld,textLengthNow,$('#oldDateTime').val());
		if (changeEle.length > 0) // display Seconds changed
			$('#timeSpanAfter').css('color', '#ebe487');
		else $('#timeSpanAfter').css('color','skyblue'); //display Seconds
			$('#timeSpanAfter').html(':' + parseInt((timeNow - changeTime) / 1000));

		return (false);
	}

	
	var interval = setInterval(autoSaveFunction, 1000);
	$('#autoSaveNumber').val(interval);
}


$('#insJapanese').on('change', () => {
	var word = $('#insJapanese').val();
	var id = $('#str3').text();
	if (!id) return;
	var area = document.getElementById(id);
	if (!area) {
  ...
  console.log('id = ',id,$('#str3').text(),$('#insJapanese').val());

  var start = area.value.substring(0,area.selectionStart);
  var selected = area.value.substring(area.selectionStart,area.selectionEnd);
  var end = area.value.substring(area.selectionEnd);

	area.value = start + word + end;
	area.focus();
  area.setSelectionRange(start.length,area.value.length - end.length);
	$('#translated').html($('#Japanese').val());//.replace(/\n/g, '<br>'));
	setTimeout(function () {
