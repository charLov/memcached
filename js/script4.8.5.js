    ...
    if (urlData.indexOf('www.dmm.com') > 0) {
      ...
      if (sp) {
        var url = '[sample Movie]\n<video width="100%" controls poster="'
          + 'https://pics.dmm.com/digital/video/' + sp[1] + '/' + sp[1] + 'pl.jpg"><source src="http://cc3001.dmm.com/litevideo/freepv/'
...
var dblClick = (ele = 'none') => {
  ...
  if (ele != 'none') {
    var area = document.getElementById($('#str3'));
//    area.focus();
    area.select();
    $('#str3').val('');
    return false;
...
var numChange = function(item){
  ...
  for (var i = 0;i < items[1];i++) {
    ...
    if ((String(i) != String(item[0])) && (String(n) == String(num))) {
...
var deleteImg = (cName) => {
  $(cName).empty();
  $.cookie('dmmImage',$('orgImage').html(),{expires: 14 , path:'/'});
}
...
//  var path  = imgUrl;
  ...
  image.onload = function(){
    var exist = $(e).html();
    if (exist.match(imgUrl)) return;
    var imgName = imgUrl.match(/.+\/(.+?\..+)/)[1].replace('.','_'); 
    if (!imgName) aModal('no imgName',imgUrl);
...
console.log(wid,hei)
    $(e).append(
      $('<div>',{class: 'clearfix ' + imgName})
        .append($('<img>',{
          src : imgUrl ,
          style: 'border: none; max-width: 300px; height: auto;',
          alt: imgName,
        }))
        .append($('<button>',{
        ...
        .append($('<span>',{
          text: ' width=' + wid + ' height=' + hei,
        }))
    );

    $.cookie('dmmImage',$(e).html(),{expires: 14 , path:'/'});
  };
