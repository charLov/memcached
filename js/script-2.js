...
var test = async function () {
	var txt = 'sougouwiki.com/search?keywords=%C1%B4%B9%BB%C0%B8%C5%CC5%BF%CD%A4%B7%A4%AB%A4%A4%A4%CA%A4%A4%C5%C4%BC%CB本作の主演は朝日りんちゃん\n';
	var res = await myConfirm(txt);
	alert(res);
	var res1 = await confirmD('Test rerera', txt);
	alert(res1);
	const result = await myPrompt("The answer is");
	alert(result);

	txt = 'abc';
	var multi = txt.split(/＿＿/);
	console.log('myConfom ends', multi);
...
function makeCode(flag, c = 'white') { // ture do false test
  ...
  if (eng.length > 0) {
    ...
    eng = eng.replace(/<\/ /g,'</').replace(/> </g,'><')
			.replace(/\(\^\s?_\s?\^;\)/g, '(^_^;)');
		txt += '<div style="border-top-style: double; border-bottom-style: double;"><button type="button" style="border: none;" class="btn btn-outline-primary" onclick="displayEnglish();">⟨impressions⟩ (in English)</button><div id="engText" style="display: none;"><br>' + eng + '<br></div></div>\n\n\n';
		txt += '<script>var displayEnglish = () => {var display = $("#engText").css("display"); if (display == "block") display = "none";	else display = "block";	$("#engText").css("display",display);} </script>'
...
var addNode = function(ap,b,max,num) {//ap=<img src=... b=check true 
  ...
  if (image) {
    ...
    img.setAttribute('data-lightbox','example-1');
		...
		else img.setAttribute('data-title',title);
...
//        img = img.replace(link.[0],'[' + )
      ...
      var altImage = '<div style="float:left;"><img ' + src[0] + alt + style;
        ...
        if ((width) && (height)) {
          if (parseInt(width[1]) > parseInt(height[1])) {
            ...
            height[1] = (size -20);
				...
				sub += snapImage(src[1]);
				//'<div style="width: ' + size + 'px; height: ' + size + 'px; float: left; display: flex; justify-content: center; align-items: center; margin: 10px; " class="snapShot"><a href="' + src[1] + '" data-lightbox="example-1"><img src="' + src[1] + '" class="snapImage" alt="' + alt + '" style="max-width:' + (size - 20) + 'px;max-height:' + (size - 20) + 'px;"></a></div>';
...
//    else {no1 = no[2]; no2 = no[3]}
  ...
  p = $('#img').val();
	if (p.indexOf('al.dmm.co.jp') < 0) $('#img').val(img + p);
...
//         ['formatting'],
	...
	.on('tbwchange', function(){
		...
		var change = 't';
		if (target) change = target.substr(0, 1);
		$('#changed').html('*' + change);
...
var titleChange = (tNo) => {
  ...
  var title = $('#' + tNo).val()
		.replace(/ （ブルーレイディスク）| （DOD）|【数量限定】|【ベストヒッツ】|【特選アウトレット】|【アウトレット】| （ブルーレイディスク）| （DOD）/g, '');
  for (var t of titles) {
...
easeSurugaya = async (urlData) => {
  ...
  if (item) {
		if ($('#title').val() == '') $('#title').val(item[1].replace(' / ', '　')
			.replace(/【数量限定】|【ベストヒッツ】|【アウトレット】|【特選アウトレット】| （ブルーレイディスク）| （DOD）/g, ''));
...
async function scrapeCaribComp(urlData) {
  ...
  if (pID) {
    ...
    var img = '<a href="http://click.dtiserv2.com/Direct/8881888-290-184037/moviepages/' 
      + pID[1] + '/index.html" target="_blank"><img src="https://www.caribbeancompr.com/moviepages/' + pID[1] + '/images/l_l.jpg" border="0" height="394" width="100%"></a>';
    if (!$('#img').val().match(img)) $('#img').val(img + $('#img').val());
    var video = '<script name="setAffplayer'
      + pID[1] + '" id="setAffplayer'
...
var myConformF = async (mes) => {
	console.log(mes)
	const result = await myConfirm(mes);
	alert(result); // ①
}
