...
/*  if(!confirm('クリアーしますか？')){
  ...
  saveTempFile(fileName,'<!-- data start ' + fileDate + ' -->\t' 
    + allToText()
    + '<!-- data end -->\n\n');
    ...
    if (urlData.indexOf('www.dmm.com') > 0) {
      ...
      if (sp) {
        var url = '<iframe type="text/html" src="https://www.dmm.com/service/digitalapi/-/html5_player/=/cid='
          ...
          + ' style="border:none;" allowfullscreen="" width="560" height="440" frameborder="0"></iframe>'
...
/*  if ($(event.relatedTarget).attr('href').match('tabA')) {// event.relatedTarget
  ...
  if ($(event.target).attr('href').match('tabA')) {// event.relatedTarget
    ...
    $('#mainTitle').text('codeMaker ver.' + version);
  } else {
...
$('#p-no-label').on('click',function(){
  var fileName = 'test/data/Harry Potter and the Philosopher.txt'
    readTempFile(fileName);


  })

$('#p-id-label').on('click',function(){
  getTempList('data');
